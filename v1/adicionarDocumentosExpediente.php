...
trait adicionarDocumentosExpediente
{
    ...
    public function adicionarDocumentosExpediente(API $api)
    {
        ...
        if ($mysqli === false) {
            $api->armarsalidaApi("9999", "Error de conexion a la BD");
        }

        $this->validarParametrosEntrantes($api, $mysqli);
        ...
        if (!file_exists($rutatmp)) {
            $api->armarsalidaApi("9999", "No fue posible cargar al servidor SII el archivo desde la url indicada", $mysqli);
        }
    ...
    private function validarParametrosEntrantes($api, $mysqli)
    {
        if (trim($_SESSION["entrada"]["idtipodoc"]) == '') {
            $api->armarsalidaApi("9999", "Debe indicarse el tipo de documento", $mysqli);
        }

        if (trim($_SESSION["entrada"]["usuario"]) == '') {
            $api->armarsalidaApi("9999", "Debe indicarse el usuario que carga el documento", $mysqli);
        }

        if (
            trim($_SESSION["entrada"]["bandeja"]) != '4.-REGMER' &&
            trim($_SESSION["entrada"]["bandeja"]) != '5.-REGESADL' &&
            trim($_SESSION["entrada"]["bandeja"]) != '6.-REGPRO'
        ) {
            $api->armarsalidaApi("9999", "Debe indicarse la bandeja adecuada", $mysqli);
        }

        if (trim($_SESSION["entrada"]["matricula"]) == '' && trim($_SESSION["entrada"]["proponente"]) == '') {
            $api->armarsalidaApi("9999", "Debe indicar el nro de la matrícula o proponente", $mysqli);
        }

        if (
            trim($_SESSION["entrada"]["tipoanexo"]) != '501' &&
            ...
            $api->armarsalidaApi("9999", "Debe indicarse un tipo anexo permitido", $mysqli);
        }
    }
}
