...
/*
 * Certifica de matrícula mercantil, en el siguiente orden:
 * 
 * - Texto del tipo de certificado
 * - Texto en caso que la matrícula esté cancelada
 * - texto en caso que la matrícula esté inactiva
 * - Indica si tiene o no documentos en trámite (Códigos de barras en trámite)
 * - Datos básicos de la matrícula (nombres, identificación, matrícula y datos comerciales
 * - Certifica de afiliación
 * - Ultima renovación
 * - Certifica de pequeña empresa Ley 1429
 * - Certifica de pequeña empresa joven Ley 1780
 * - Certifica de Cancelacion (fechacancelacion <> "" o actos (0530 al 0540)
 * - Transformaciones (acto 0400)
 * - Actos del libro XIX
 * - Cambio de jurisdicción (Acto 9997)
 * - Cambio de domicilio 
 * - Notificación judicial
 * - Certifica sitios web (Acto 4000)
 * - Propietarios
 * - Reseña a casa principal
 * - Poderes (Certifica 1500)
 * - Administrador (Vínculo 2600)
 * - Arrendatariof
 * - Oposición a la enajenación (acto 0192)
 * - Liquidación obligatoria, actos 0650 al 0690
 * - Actividad
 * - Financiera
 * - Embargos
 * - Establecimientos
 * - Certificas textuales (certifica 1940)
 */

function generarCertificadosPdfMatriculaSii2($mysqli, $data, $tipo, $valorCertificado = 0, $operacion = '', $recibo = '', $aleatorio = '', $certificadoConsultaRues = 'no', $escajero = 'SI', $esbanco = 'NO', $firmar = '') {
    ...
    //
    $_SESSION["generales"]["fcorte"] = retornarRegistroMysqli2($mysqli, 'mreg_cortes_renovacion', "ano='" . date("Y") . "'", "corte");
    $_SESSION["generales"]["fcortemesdia"] = substr($_SESSION["generales"]["fcorte"],4,4);
    
    $nameLog = 'generarCertificadosPdfMatriculaSii_' . date("Ymd");
...
// Arma lista de certificas y su clasificacion
    ...
    $temcerts = retornarRegistrosMysqli2($mysqli, 'mreg_codigos_certificas', "1=1", "id");
...
// Arma orden del certificado
    ...
    $temcerts = retornarRegistrosMysqli2($mysqli, 'mreg_orden_certificas', "tipocertificado='CerMat'", "orden");
    $i = 0;
    foreach ($temcerts as $c) {
        $i++;
...
//
            ...
            public function Header() {
                ...
                if ($this->tituloEstadoMatricula != 'MF' && $this->tituloEstadoMatricula != 'MC') {
                    if (date("Ymd") <= $_SESSION["generales"]["fcorte"]) {
                        ...
                        $txt .= 'RENUEVE SU MATRÍCULA A MÁS TARDAR EL ' . \funcionesSii2::mostrarFechaLetras1($_SESSION["generales"]["fcorte"]) . ' Y EVITE SANCIONES DE HASTA 17 S.M.L.M.V';
                        $this->writeHTML($txt, true, false, true, false, 'C');
                    } else {
                        $this->Ln();
                        $this->Ln();
    ...
    // ****************************************************************************** //    
    if (convertirMayusculasSii2($data["organizacion"], $data["categoria"])) {
        $data["crt1"] = array();
                ...
                if ($data["disueltaporvencimiento"] == 'si') {
                    ...
                    if ($data["disueltaporacto510"] == 'si') {
                        ...
                        if ($data["perdidacalidadcomerciante"] == 'si') {
                            ...
                            $fcorte1 = retornarRegistroMysqli2($mysqli, 'mreg_cortes_renovacion', "ano='" . $ano1 . "'", "corte");
...
// *************************************************************************** //
    ...
    if ($aleatorio == '') {
        ...
        if ($certificadoConsultaRues != 'si') {
            $msg = 'Recibo : ' . $recibo . ' | Usuario : ' . $_SESSION["generales"]["codigousuario"] . '(' . $escajero . ') (' . $esbanco . ') | Matricula : ' . $data["matricula"] . ' | Tipo : CerMat | Hora : ' . date("His");
...
function generarCertificadosPdfExistenciaSii2($mysqli, $data, $tipo, $valorCertificado = 0, $operacion = '', $recibo = '', $aleatorio = '', $certificadoConsultaRues = 'no', $escajero = 'SI', $esbanco = 'NO', $firmar = '') {
    ...
    // Arma orden del certificado
    $_SESSION["generales"]["ordencerti"] = array();
    $temcerts = retornarRegistrosMysqli2($mysqli, 'mreg_orden_certificas', "tipocertificado='CerExi'", "orden");
    $i = 0;
    foreach ($temcerts as $c) {
        $i++;
        if (strtoupper($c["estado"]) == 'A') {
    ...
    if (!class_exists('PDFReque')) {
        ...
        class PDFReque extends TCPDF {
            ...
            public function Header() {
                ...
                if ($this->tituloEstadoMatricula != 'MF' &&
...
function generarCertificadosPdfEsadlSii2($mysqli, $data, $tipo, $valorCertificado = 0, $operacion = '', $recibo = '', $aleatorio = '', $certificadoConsultaRues = 'no', $escajero = 'SI', $esbanco = 'NO', $firmar = '') {
    ...
    $nameLog = 'generarCertificadosPdfEsadlSii_' . date("Ymd");
    ...
    // Arma orden del certificado
    $_SESSION["generales"]["ordencerti"] = array();
    $temcerts = retornarRegistrosMysqli2($mysqli, 'mreg_orden_certificas', "tipocertificado='CerEsadl'", "orden");
    $i = 0;
    foreach ($temcerts as $c) {
        $i++;
        if (strtoupper($c["estado"]) == 'A') {
    ...
    if (
        ...
        if ($data["ctrcancelacion1429"] != '3') {
            if (date("md") > $_SESSION["generales"]["fcorte"]) {
                if ($data["ultanoren"] == (date("Y") - 1)) {
                    $pdf->norenovado = 'si';
                    if ($data["disueltaporvencimiento"] == 'si') {
                        $pdf->norenovado = 'no';
    ...
    if (!empty($data["lcodigosbarras"])) {
        ...
        $pdf->Ln();
        $pdf->SetTextColor(0, 0, 0);
        //WSIERRA : 2019-01-30 : Se incluye invocación de log propia del API 
        \logSii2::general2($nameLog, $operacion . '-' . $data["matricula"], 'Imprimio que tiene codigos de barras pendientes');
        //log::general2($nameLog, $operacion . '-' . $data["matricula"], 'Imprimio que tiene codigos de barras pendientes');
    }
    ...
    $pdf->Ln();
    //WSIERRA : 2019-01-30 : Se incluye invocación de log propia del API 
    \logSii2::general2($nameLog, $operacion . '-' . $data["matricula"], 'Imprimio titulo certificado');
    //log::general2($nameLog, $operacion . '-' . $data["matricula"], 'Imprimio titulo certificado');
    // *************************************************************************** //
    // Título de certifica general
    // *************************************************************************** //    
    $pdf->SetFont('courier', '', $pdf->tamanoLetra);
    ...
    if ($data["estadomatricula"] == 'IC' ||
        ...
        $pdf->SetTextColor(0, 0, 0);
        //WSIERRA : 2019-01-30 : Se incluye invocación de log propia del API 
        \logSii2::general2($nameLog, $operacion . '-' . $data["matricula"], 'Imprimio texto matricula cancelada');
        //log::general2($nameLog, $operacion . '-' . $data["matricula"], 'Imprimio texto matricula cancelada');
    }
    ...
    if ($aleatorio == '') {
         ...
         */
        if ($certificadoConsultaRues != 'si') {
            $msg = 'Recibo : ' . $recibo . ' | Usuario : ' . $_SESSION["generales"]["codigousuario"] . '(' . $escajero . ') (' . $esbanco . ') | Matricula : ' . $data["matricula"] . ' | Tipo : CerEsadl | Hora : ' . date("His");
            $nameLog1 = 'controlFirmaCertificados_' . date("Ymd");
            if (defined('ACTIVAR_FIRMA_DIGITAL_CERTIFICADOS')) {
                if ((ACTIVAR_FIRMA_DIGITAL_CERTIFICADOS == 'SI' && $escajero != 'SI') ||
                        ACTIVAR_FIRMA_DIGITAL_CERTIFICADOS == 'SI-TODOS' || (ACTIVAR_FIRMA_DIGITAL_CERTIFICADOS == 'SI' && $firmar == 'si')) {
...
function generarCertificadosPdfLibrosSii2($mysqli, $data, $tipo, $valorCertificado = 0, $operacion = '', $recibo = '', $aleatorio = '', $tipoCertificado = '', $certificadoConsultaRues = 'no', $escajero = 'SI', $esbanco = 'NO', $firmar = '') {
    ...
    if ($aleatorio == '') {
        ...
        if ($certificadoConsultaRues != 'si') {
            if (defined('ACTIVAR_FIRMA_DIGITAL_CERTIFICADOS')) {
                if ((ACTIVAR_FIRMA_DIGITAL_CERTIFICADOS == 'SI' && $escajero == 'NO') ||
                        ACTIVAR_FIRMA_DIGITAL_CERTIFICADOS == 'SI-TODOS' || (ACTIVAR_FIRMA_DIGITAL_CERTIFICADOS == 'SI' && $firmar == 'si')) {
                    require_once('PDFA.class.php');
                    $ins = new PDFA();
                    $ins->generarPDFAfirmado($aleatorio, $name1, 'no');
...
function armarDatosBasicos($pdf, $data, $titulo = '', $imprimirtitulo = 'si', $mysqli = null) {
    ...
    if (defined('MAYUSCULAS_SOSTENIDAS') && MAYUSCULAS_SOSTENIDAS == 'SI') {
        $txt .= '</strong>' . strtoupper(retornarRegistroMysqli2($mysqli, 'bas_organizacionjuridica', "id='" . $data["organizacion"] . "'", "descripcion"));
    } else {
        $txt .= '</strong>' . retornarRegistroMysqli2($mysqli, 'bas_organizacionjuridica', "id='" . $data["organizacion"] . "'", "descripcion");
    }
    $pdf->writeHTML($txt, true, false, true, false, 'L');
    ...
    if ($data["organizacion"] == '01') {
        if (defined('MAYUSCULAS_SOSTENIDAS') && MAYUSCULAS_SOSTENIDAS == 'SI') {
            $txt = '<strong>IDENTIFICACIÓN : </strong>' . strtoupper(retornarRegistroMysqli2($mysqli, 'mreg_tipoidentificacion', "id='" . $data["tipoidentificacion"] . "'", "descripcion") . ' - ' . $data["identificacion"]);
        } else {
            $txt = '<strong>Identificación : </strong>' . retornarRegistroMysqli2($mysqli, 'mreg_tipoidentificacion', "id='" . $data["tipoidentificacion"] . "'", "descripcion") . ' - ' . $data["identificacion"];
        }
        $pdf->writeHTML($txt, true, false, true, false, 'L');
    }
    ...
    if ($data["organizacion"] == '01' || ($data["organizacion"] > '02' && $data["categoria"] == '1')) {
        if (ltrim($data["nit"], "0") != '') {
            $sp = \funcionesSii2::separarDv($data["nit"]);
            if (defined('MAYUSCULAS_SOSTENIDAS') && MAYUSCULAS_SOSTENIDAS == 'SI') {
            ...
            if (ltrim(trim($data["admondian"]), "0") != '') {
                if (defined('MAYUSCULAS_SOSTENIDAS') && MAYUSCULAS_SOSTENIDAS == 'SI') {
                    $txt = '<strong>ADMINISTRACIÓN DIAN : </strong>' . strtoupper(retornarRegistroMysqli2($mysqli, 'bas_admindian', "id='" . $data["admondian"] . "'", "descripcion"));
                } else {
                    $txt = '<strong>Administración DIAN : </strong>' . retornarRegistroMysqli2($mysqli, 'bas_admindian', "id='" . $data["admondian"] . "'", "descripcion");
                }
                $pdf->writeHTML($txt, true, false, true, false, 'L');
            }
        } else {
            if ($data["estadonit"] == '4') {
                ...
                if (diferenciaEntreFechaBase30CertificadosSii2(date("Ymd"), $data["fechamatricula"]) > 2) {
                    if (defined('MAYUSCULAS_SOSTENIDAS') && MAYUSCULAS_SOSTENIDAS == 'SI') {
    ...
    if ($_SESSION["generales"]["MAYUSCULAS_SOSTENIDAS"]) {
        $txt = '<strong>DOMICILIO : </strong>' . retornarNombreMunicipioMysqli2($mysqli, $data["muncom"]);
    } else {
        $txt = '<strong>Domicilio : </strong>' . retornarNombreMunicipioMysqli2($mysqli, $data["muncom"]);
    }
    $pdf->writeHTML($txt, true, false, true, false, 'L');
...
function armarCertificaMatriculaInscripcion($pdf, $data, $titulo = '', $imprimirtitulo = 'si', $mysqli = null) {
    ...
    $txt .= '<strong>' . $titf . ' : </strong>' . strtoupper(\funcionesSii2::mostrarFechaLetras($data["fechamatricula"])) . '<br>';
    $txt .= '<strong>ULTIMO AÑO RENOVADO  : </strong>' . $data["ultanoren"] . '<br>';
    $txt .= '<strong>' . $titr . ' : </strong>' . strtoupper(\funcionesSii2::mostrarFechaLetras($data["fecharenovacion"])) . '<br>';
    if ($data["organizacion"] == '01' || ($data["organizacion"] > '02' && $data["categoria"] == '1')) {
        $txt .= '<strong>ACTIVO TOTAL  : </strong>' . number_format($data["acttot"], 2) . '<br>';
        if (trim($data["gruponiif"]) != '') {
            $txt .= '<strong>GRUPO NIIF  : </strong>' . retornarRegistroMysqli2($mysqli, 'bas_gruponiif', "id='" . $data["gruponiif"] . "'", "descripcion") . '<br>';
        }
    } else {
...
function armarUbicacionDatosGenerales($pdf, $data, $titulo = '', $imprimirtitulo = 'si', $mysqli = null) {
    ...
    if (trim($data["barriocom"]) != '') {
        $txt .= '<strong>BARRIO  : </strong>' . retornarRegistroMysqli2($mysqli, 'mreg_barriosmuni', "idmunicipio='" . $data["muncom"] . "' and idbarrio='" . $data["barriocom"] . "'", "nombre") . '<br>';
    }
    $txt .= '<strong>MUNICIPIO  / DOMICILIO: </strong>' . $data["muncom"] . ' - ' . retornarRegistroMysqli2($mysqli, 'bas_municipios', "codigomunicipio='" . $data["muncom"] . "'", "ciudad") . '<br>';
    ...
    if ($data["organizacion"] != '02' && $data["categoria"] != '3') {
        if (trim($data["dirnot"]) != '' ||
            ...
            $txt .= '<strong>MUNICIPIO  : </strong>' . $data["munnot"] . ' - ' . retornarRegistroMysqli2($mysqli, 'bas_municipios', "codigomunicipio='" . $data["munnot"] . "'", "ciudad") . '<br>';
            if (trim($data["barrionot"]) != '') {
                $txt .= '<strong>BARRIO  : </strong>' . retornarRegistroMysqli2($mysqli, 'mreg_barriosmuni', "idmunicipio='" . $data["munnot"] . "' and idbarrio='" . $data["barrionot"] . "'", "nombre") . '<br>';
            }
...
function armarDatosBasicosLibros($pdf, $data, $mysqli = null) {
    ...
    $txt = '<strong>ORGANIZACION JURIDICA: </strong>' . strtoupper(retornarRegistroMysqli2($mysqli, 'bas_organizacionjuridica', "id='" . $data["organizacion"] . "'", "descripcion"));
    $pdf->writeHTML($txt, true, false, true, false, 'L');
    if ($data["organizacion"] > '02') {
        switch ($data["categoria"]) {
            case "1":
...
function armarLibros($pdf, $data, $mysqli = null) {
    ...
    $txt1 = '';
    $tieneLibros = 'NO';
    $data["inscripciones"] = \funcionesSii2::ordenarMatriz($data["inscripciones"], "freg");

    foreach ($data["inscripciones"] as $ix) {
        if ($ix["grupoacto"] == '004' || $ix["grupoacto"] == '059' ||
                ($ix["lib"] == 'RM07' || $ix["lib"] == 'RE52') && (ltrim(trim($ix["acto"]), "0") == '')) {
            $txt2 = 'NO DEFINIDO';
            if (trim($ix["deslib"]) != '') {
                $txt2 = trim($ix["deslib"]);
            } else {
            ...
            $txt1 .= '<td width="13%">' . \funcionesSii2::mostrarFecha($ix["freg"]) . '</td>';
    ...
    /*
      foreach ($data["inscripciones"] as $ix) {
      if ($ix["grupoacto"] == '004' || $ix["grupoacto"] == '059' || ($ix["lib"] == 'RM07' || $ix["lib"] == 'RE52') && (ltrim(trim($ix["acto"]), "0") == '')) {
      $txt2 = 'NO DEFINIDO';
      if (trim($ix["deslib"]) != '') {
      $txt2 = trim($ix["deslib"]);
      } else {
      ...
      $txt1 .= '<td width="14%">' . $ix["lib"] . '-' . $ix["nreg"] . '</td>';
      $txt1 .= '<td width="13%">' . mostrarFechaSii2($ix["freg"]) . '</td>';
      $txt1 .= '<td width="13%">' . number_format($ix["numhojas"], 0) . '</td>';
      $txt1 .= '</tr>';
      }
      }
        ...
        if ($tieneadministrador > 0) {
            ...
            foreach ($data["vinculos"] as $v) {
                if ($v["tipovinculo"] == 'ADMP' || $v["tipovinculo"] == 'ADMS1' || $v["tipovinculo"] == 'ADMS2') {
                    ...
                    $txt .= '<strong>IDENTIFICACION : </strong>' . retornarRegistroMysqli2($mysqli, 'mreg_tipoidentificacion', "id='" . $v["idtipoidentificacionotros"] . "'", "descripcion") . ' - ' . $v["identificacionotros"] . '<br>';
                    $txt .= '<strong>VINCULACION : </strong>' . retornarRegistroMysqli2($mysqli, 'mreg_codvinculos', "id='" . $v["vinculootros"] . "'", "descripcion") . '<br>';
                    $txt .= '<strong>FECHA DE REGISTRO DE LA VINCULACION : </strong>' . strtoupper(\funcionesSii2::mostrarFechaLetras($v["fechaotros"])) . '<br>';
        ...
        if ($tienearrendatario > 0) {
            ...
            foreach ($data["vinculos"] as $v) {
                if ($v["tipovinculo"] == 'ARR') {
                    ...
                    $txt .= '<strong>IDENTIFICACION : </strong>' . retornarRegistroMysqli2($mysqli, 'mreg_tipoidentificacion', "id='" . $v["idtipoidentificacionotros"] . "'", "descripcion") . ' - ' . $v["identificacionotros"] . '<br>';
                    $txt .= '<strong>VINCULACION : </strong>' . retornarRegistroMysqli2($mysqli, 'mreg_codvinculos', "id='" . $v["vinculootros"] . "'", "descripcion") . '<br>';
                    $txt .= '<strong>FECHA DE REGISTRO DEL CONTRATO : </strong>' . strtoupper(\funcionesSii2::mostrarFechaLetras($v["fechaotros"])) . '<br>';
                    $txt .= '<strong>LIBRO Y NÚMERO DE INSCRIPCIÓN : </strong>' . $v["librootros"] . ' - ' . $v["inscripcionotros"] . '<br>';
                    $pdf->writeHTML($txt, true, false, true, false, 'L');
                    $pdf->Ln();
                }
...
// *************************************************************************** //
function armarCertificaPequenaEmpresaJoven($pdf, $data, $mysql = null) {
    ...
    if ($mipyme == 'si') {
        if (date("Y") > substr($data["fechamatricula"], 0, 4)) {
            if ($data["cumplerequisitos1780"] == 'N' ||
...
function verificarTamanoMipyme($mysqli = null, $data = array()) {
    ...
    $salmin = localizarSmmlvMysqli2($mysqli, $fec);
    if ($salmin == 0) {
        return 'no';
    } else {
    ...
    if (substr($data["fechamatricula"], 0, 4) == date("Y")) {
        ...
        if (date("md") > $_SESSION["generales"]["fcortemesdia"]) {
            if ($data["ultanoren"] == date("Y")) {
                if ($data["fecharenovacion"] <= date("Y") . $_SESSION["generales"]["fcortemesdia"]) {
                    $mipyme = 'si';
                }
            }
        } else {
...
function armarCertificaPersoneriaJuridica($pdf, $data, $mysqli = null) {
    ...
    if ($tienepersoneria == 'si') {
        ...
        $txt .= strtoupper(\funcionesSii2::mostrarFechaLetras1($data["fecperj"])) . ' BAJO EL NÚMERO ' . $data["numperj"] . ' ';
        if (ltrim(trim($data["idorigenperj"]), "0") != '') {
            if (strlen(trim($data["idorigenperj"])) == 5) {
                $txt .= 'OTORGADA POR ' . retornarRegistroMysqli2($mysqli, "mreg_tablassirep", "idtabla='43' and idcodigo='" . $data["idorigenperj"] . "'", "descripcion");
            } else {
...
function armarCertificaSitControl($pdf, $data, $mysqli = null) {
                ...
                if ($i["crev"] != '1' && $i["crev"] != '9') {
                    ...
                    if ($vinculoindividualizado == 'no') {
                        ...
                        if ($tipoSit != '') {
                            ...
                            if (trim($data["muncom"]) != '') {
                                $txt .= 'MUNICIPIO : ' . retornarNombreMunicipioMysqli2($mysqli, $data["muncom"]) . '<br>';
                            }
                            if (trim($data["paicom"]) != '') {
                                $txt .= 'PAIS : ' . retornarNombrePaisMysqli2($mysqli, $data["paicom"]) . '<br>';
                            }
                        }
                    }
                        ...
                        if (
                            ...
                            if ($tipoSit != '') {
                                if (trim($v["identificacionotros"]) != '') {
                                    $tx = retornarRegistroMysqli2($mysqli, 'mreg_est_inscritos', "numid='" . $v["identificacionotros"] . "' and (ctrestmatricula='MA' or ctrestmatricula='MI' or ctrestmatricula='II' or ctrestmatricula='IA')");
                                    if ($tx && !empty($tx)) {
                                        ...
                                        if (trim($tx["muncom"]) != '') {
                                            if ($tx["muncom"] == '00000' || $tx["muncom"] == '99999') {
                                                ...
                                                $txt .= 'MUNICIPIO : ' . $tx["muncom"] . ' - ' . retornarNombreMunicipioMysqli2($mysqli, $tx["muncom"]) . '<br>';
                                            }
                                        }
                                        if (trim($tx["dircom"]) != '') {
                                        ...
                                        if (trim($tx["paicom"]) != '') {
                                            $tpai = retornarRegistroMysqli2($mysqli, 'bas_paises', "idpais='" . $tx["paicom"] . "'", "nombrepais");
                                            if ($tpai == '') {
                                                $tpai = retornarRegistroMysqli2($mysqli, 'bas_paises', "codnumpais='" . $tx["paicom"] . "'", "nombrepais");
                                            }
                                            if ($tpai == '') {
                                                $tpai = 'Colombia';
                                            }
                                        ...
                                        if (trim($tx["ciiu1"]) != '') {
                                            $txt .= 'CIIU : ' . $tx["ciiu1"] . ' - ' . retornarDescripcionCiiuMysqli2($mysqli, $tx["ciiu1"]) . '<br>';
                                        }
                                        if ($tx["ciiu2"] != '') {
                                            $txt .= 'CIIU : ' . $tx["ciiu2"] . ' - ' . retornarDescripcionCiiuMysqli2($mysqli, $tx["ciiu2"]) . '<br>';
                                        }
                                        if ($tx["ciiu3"] != '') {
                                            $txt .= 'CIIU : ' . $tx["ciiu3"] . ' - ' . retornarDescripcionCiiuMysqli2($mysqli, $tx["ciiu3"]) . '<br>';
                                        }
                                        if ($tx["ciiu4"] != '') {
                                            $txt .= 'CIIU : ' . $tx["ciiu4"] . ' - ' . retornarDescripcionCiiuMysqli2($mysqli, $tx["ciiu4"]) . '<br>';
                                        }
                                        if ($tx["desactiv"] != '') {
                                            $txt .= 'DESCRIPCIÓN DE LA ACTIVIDAD : ' . $tx["desactiv"] . '<br>';
                                        }
                                    } else {
                                        ...
                                        if (trim($v["municipiootros"]) != '') {
                                            if ($v["municipiootros"] == '00000' || $v["municipiootros"] == '99999') {
                                                ...
                                                $txt .= 'MUNICIPIO : ' . $v["municipiootros"] . ' - ' . retornarNombreMunicipioMysqli2($mysqli, $v["municipiootros"]) . '<br>';
                                            }
                                        }
                                        ...
                                        if (trim($v["paisotros"]) != '') {
                                            $tpai = retornarRegistroMysqli2($mysqli, 'bas_paises', "idpais='" . $v["paisotros"] . "'", "nombrepais");
                                            if ($tpai == '') {
                                                $tpai = retornarRegistroMysqli2($mysqli, 'bas_paises', "codnumpais='" . $v["paisotros"] . "'", "nombrepais");
                                            }
                                            if ($tpai == '') {
                                                $tpai = 'Colombia';
                                            }
                                        ...
                                        if (trim($v["ciiu1"]) != '') {
                                            $txt .= 'CIIU : ' . $v["ciiu1"] . ' - ' . retornarDescripcionCiiuMysqli2($mysqli, $v["ciiu1"]) . '<br>';
                                        }
                                        if ($v["ciiu2"] != '') {
                                            $txt .= 'CIIU : ' . $v["ciiu2"] . ' - ' . retornarDescripcionCiiuMysqli2($mysqli, $v["ciiu2"]) . '<br>';
                                        }
                                        if ($v["ciiu3"] != '') {
                                            $txt .= 'CIIU : ' . $v["ciiu3"] . ' - ' . retornarDescripcionCiiuMysqli2($mysqli, $v["ciiu3"]) . '<br>';
                                        }
                                        if ($v["ciiu4"] != '') {
                                            $txt .= 'CIIU : ' . $v["ciiu4"] . ' - ' . retornarDescripcionCiiuMysqli2($mysqli, $v["ciiu4"]) . '<br>';
                                        }
                                        if ($v["desactiv"] != '') {
                                    ...
                                    if ($v["fechaconfiguracion"] != '') {
                                        $txt .= 'FECHA DE CONFIGURACION DE LA SITUACIÓN : ' . \funcionesSii2::mostrarFecha($v["fechaconfiguracion"]) . '<br>';
                                    }
                                }
                            }
                        }
    ...
    if ($tienesitcontrol == 'no') {
        $resx = armarCertificaTextoLibreClase($pdf, $data, 'CRT-SITCONTROL', 'CERTIFICA - SITUACIONES DE CONTROL Y GRUPOS EMPRESARIALES');
        return $resx;
    } else {
        return true;
    }
...
// *************************************************************************** //
function armarCertificaSitControlAnterior($pdf, $data, $mysqli = null) {
    //
    $pdf->SetFont('courier', '', $pdf->tamanoLetra);
    ...
    foreach ($data["inscripciones"] as $i) {
        if ($i["grupoacto"] == '022') {
            ...
            if ($i["crev"] != '1' && $i["crev"] != '9') {
                foreach ($data["vinculos"] as $v) {
                    if ((trim($v["dupliotros"]) == '' && $v["librootros"] == $i["lib"] && $v["inscripcionotros"] == $i["nreg"]) || (trim($v["dupliotros"]) != '' && $v["librootros"] == $i["lib"] && $v["inscripcionotros"] == $i["nreg"] && $v["dupliotros"] == $i["dupli"])) {
                        if ($v["tipovinculo"] == 'SCMAI' ||
                                $v["tipovinculo"] == 'SCCOI' ||
                                $v["tipovinculo"] == 'SISUI' ||
                                $v["tipovinculo"] == 'SICNI' ||
                                $v["tipovinculo"] == 'SITC1' ||
                ...
                if ($i["crev"] != '1' && $i["crev"] != '9') {
                    ...
                    if ($vinculoindividualizado == 'no') {
                        ...
                        if ($tipoSit != '') {
                            $txt .= '<br><strong>** ' . $tipoSit . ' : </strong>' . $data["nombre"] . '<br>';
                            if (trim($data["muncom"]) != '') {
                                $txt .= 'MUNICIPIO : ' . retornarRegistroMysqli2($mysqli, 'bas_municipios', "codigomunicipio='" . $data["muncom"] . "'", "ciudad") . '<br>';
                            }
                            if (trim($data["paicom"]) != '') {
                                $txt .= 'PAIS : ' . \funcionesSii2::retornarNombrePais($mysqli, $data["paicom"]) . '<br>';
                            }
                        }
                    }
                        ...
                        if ((trim($v["dupliotros"]) == '' && $v["librootros"] == $i["lib"] && $v["inscripcionotros"] == $i["nreg"]) || (trim($v["dupliotros"]) != '' && $v["librootros"] == $i["lib"] && $v["inscripcionotros"] == $i["nreg"] && $v["dupliotros"] == $i["dupli"])) {
                            ...
                            if ($tipoSit != '') {
                                if (trim($v["identificacionotros"]) != '') {
                                    ...
                                    if ($tx && !empty($tx)) {
                                        ...
                                        if (trim($tx["muncom"]) != '') {
                                            if ($tx["muncom"] == '00000' || $tx["muncom"] == '99999') {
                                                ...
                                                $txt .= 'MUNICIPIO : ' . $tx["muncom"] . ' - ' . retornarRegistroMysqli2($mysqli, 'bas_municipios', "codigomunicipio='" . $tx["muncom"] . "'", "ciudad") . '<br>';
                                            }
                                        }
                                        if (trim($tx["dircom"]) != '') {
                                            $txt .= 'DIRECCIÓN : ' . $tx["dircom"] . '<br>';
                                        ...
                                        if (trim($tx["paicom"]) != '') {
                                            $tpai = \funcionesSii2::retornarNombrePais($mysqli, $tx["paicom"]);
                                            if ($tpai == '') {
                                                $tpai = 'Colombia';
                                            }
                                            $txt .= 'PAIS : ' . $tpai . '<br>';
                                        ...
                                        if (trim($tx["ciiu1"]) != '') {
                                            $txt .= 'CIIU : ' . $tx["ciiu1"] . ' - ' . \funcionesSii2::retornarDescripcionCiiu($mysqli, $tx["ciiu1"]) . '<br>';
                                        }
                                        if ($tx["ciiu2"] != '') {
                                            $txt .= 'CIIU : ' . $tx["ciiu2"] . ' - ' . \funcionesSii2::retornarDescripcionCiiu($mysqli, $tx["ciiu2"]) . '<br>';
                                        }
                                        if ($tx["ciiu3"] != '') {
                                            $txt .= 'CIIU : ' . $tx["ciiu3"] . ' - ' . \funcionesSii2::retornarDescripcionCiiu($mysqli, $tx["ciiu3"]) . '<br>';
                                        }
                                        if ($tx["ciiu4"] != '') {
                                            $txt .= 'CIIU : ' . $tx["ciiu4"] . ' - ' . \funcionesSii2::retornarDescripcionCiiu($mysqli, $tx["ciiu4"]) . '<br>';
                                        }
                                        if ($tx["desactiv"] != '') {
                                            $txt .= 'DESCRIPCIÓN DE LA ACTIVIDAD : ' . $tx["desactiv"] . '<br>';
                                        }
                                        ...
                                        if (trim($v["municipiootros"]) != '') {
                                            if ($v["municipiootros"] == '00000' || $v["municipiootros"] == '99999') {
                                                ...
                                                $txt .= 'MUNICIPIO : ' . $v["municipiootros"] . ' - ' . retornarRegistroMysqli2($mysqli, "codigomunicipio='" . $v["municipiootros"] . "'", "ciudad") . '<br>';
                                            }
                                        }
                                        if (trim($v["direccionotros"]) != '') {
                                            $txt .= 'DIRECCIÓN : ' . $v["direccionotros"] . '<br>';
                                        ...
                                        if (trim($v["paisotros"]) != '') {
                                            $tpai = retornarRegistro('bas_paises', "idpais='" . $v["paisotros"] . "'", "nombrepais");
                                            if ($tpai == '') {
                                                $tpai = retornarRegistro('bas_paises', "codnumpais='" . $v["paisotros"] . "'", "nombrepais");
                                            }
                                            if ($tpai == '') {
                                                $tpai = 'Colombia';
                                            }
                                        ...
                                        if (trim($v["ciiu1"]) != '') {
                                            $txt .= 'CIIU : ' . $v["ciiu1"] . ' - ' . \funcionesSii2::retornarDescripcionCiiu($mysqli, $v["ciiu1"]) . '<br>';
                                        }
                                        if ($v["ciiu2"] != '') {
                                            $txt .= 'CIIU : ' . $v["ciiu2"] . ' - ' . \funcionesSii2::retornarDescripcionCiiu($mysqli, $v["ciiu2"]) . '<br>';
                                        }
                                        if ($v["ciiu3"] != '') {
                                            $txt .= 'CIIU : ' . $v["ciiu3"] . ' - ' . \funcionesSii2::retornarDescripcionCiiu($mysqli, $v["ciiu3"]) . '<br>';
                                        }
                                        if ($v["ciiu4"] != '') {
                                            $txt .= 'CIIU : ' . $v["ciiu4"] . ' - ' . \funcionesSii2::retornarDescripcionCiiu($mysqli, $v["ciiu4"]) . '<br>';
                                        }
                                        if ($v["desactiv"] != '') {
                                            $txt .= 'DESCRIPCIÓN DE LA ACTIVIDAD : ' . $v["desactiv"] . '<br>';
                                        }
                                    ...
                                    if ($v["fechaconfiguracion"] != '') {
                                        $txt .= 'FECHA DE CONFIGURACION DE LA SITUACIÓN : ' . mostrarFechaSii2($v["fechaconfiguracion"]) . '<br>';
                                    }
                                }
                            }
                        }
                                    ...
                                    if ($v["vinculootros"] == '6000' || $v["vinculootros"] == '6002') {
                                        if (trim($v["identificacionotros"]) != '') {
                                            ...
                                            if ($tx && !empty($tx)) {
                                                ...
                                                if (trim($tx["muncom"]) != '') {
                                                    $txt .= 'MUNICIPIO : ' . retornarRegistroMysqli2($mysqli, "bas_municipios", "codigomunicipio='" . $tx["muncom"] . "'", "ciudad") . '<br>';
                                                }
                                                if (trim($tx["dircom"]) != '') {
                                                    $txt .= 'DIRECCIÓN : ' . $tx["dircom"] . '<br>';
                                                }
                                                if (trim($tx["paicom"]) != '') {
                                                    $txt .= 'PAIS : ' . strtoupper(\funcionesSii2::retornarNombrePais($mysqli, $tx["paicom"])) . '<br>';
                                                }
                                                if (trim($tx["ciiu1"]) != '') {
                                                    $txt .= 'CIIU : ' . $tx["ciiu1"] . ' - ' . \funcionesSii2::retornarDescripcionCiiu($mysqli, $tx["ciiu1"]) . '<br>';
                                                }
                                                ...
                                                if (trim($v["municipiootros"]) != '') {
                                                    $txt .= 'MUNICIPIO : ' . retornarRegistroMysqli2($mysqli, "bas_municipios", "codigomunicipio='" . $v["municipiootros"] . "'", "ciudad") . '<br>';
                                                }
                                                if (trim($v["direccionotros"]) != '') {
                                                    $txt .= 'DIRECCIÓN : ' . $v["direccionotros"] . '<br>';
                                                }
                                                if (trim($v["paisotros"]) != '') {
                                                    $txt .= 'PAIS : ' . strtoupper(\funcionesSii2::retornarNombrePais($mysqli, $v["paisotros"])) . '<br>';
                                                }
                                                if (trim($v["ciiu1"]) != '') {
                                                    $txt .= 'CIIU : ' . $v["ciiu1"] . ' - ' . \funcionesSii2::retornarDescripcionCiiu($mysqli, $v["ciiu1"]) . '<br>';
                                                }
...
function armarCertificaCapitalSat($pdf, $data, $mysqli = null) {
    armarCertificaTextoLibreClase($pdf, $data, 'CRT-CAPSAT', 'CERTIFICA - CAPITAL SOCIEDADES AGRARIAS DE TRANSFORMACIÓN', $mysqli);
}
...
function armarCertificaReformas($pdf, $data, $mysqli = null) {
    ...
    // 2019-09-06: JINT: Certificas en formato HTML
        ...
        if ($ins["esreforma"] == 'S') {
            ...
            if ($ins["crev"] != '1') {
                ...
                if ($ins["idoridoc"] != '' && $ins["idoridoc"] != '000000' && $ins["idoridoc"] != '999999') {
                    $txto = retornarRegistroMysqli2($mysqli, 'mreg_tablassirep', "idtabla='51' and idcodigo='" . $ins["idoridoc"] . "'", "descripcion");
                    if ($txto == '') {
                        ...
                        $txt .= '<td width="23%">' . retornarRegistroMysqli2($mysqli, 'mreg_tablassirep', "idtabla='51' and idcodigo='" . $ins["idoridoc"] . "'", "descripcion");
                    }
                } else {
                    if (strtoupper(trim($ins["txoridoc"])) == 'NO TIENE NO TIENE') {
                        $ins["txoridoc"] = 'ORGANOS DE ADMINISTRACION';
                ...
                $txt .= '<td width="2%">&nbsp;</td>';
                $txt .= '<td width="10%">' . substr(retornarRegistroMysqli2($mysqli, "bas_municipios", "codigomunicipio='" . $ins["idmunidoc"] . "'", "ciudad"), 0, 15) . '</td>';
                $txLib = '';
                switch ($ins["lib"]) {
                    case "RE51":
                        $txLib = 'RE01';
                ...
                foreach ($data["vinculos"] as $v) {
                    if ($v["vinculootros"] == $tiposocios) {
                        ...
                        if ($v["idtipoidentificacionotros"] == '2') {
                            $sp = \funcionesSii2::separarDv($v["identificacionotros"]);
...
function armarCertificaResenaCasaPrincipal($pdf, $data, $mysqli = null) {
    ...
    if ($data["organizacion"] == '08' || ($data["categoria"] == '2' || $data["categoria"] == '3')) {
        ...
        if ($data["cpcodcam"] != '') {
            $sp = \funcionesSii2::separarDv($data["cpnumnit"]);
        ...
        if ($data["cpcodmun"] != '') {
            if ($data["cpcodmun"] != '99999') {
                $txt .= '<strong>DOMICILIO : </strong>' . retornarNombreMunicipioMysqli2($mysqli, $data["cpcodmun"]) . '<br>';
            } else {
                $txt .= '<strong>DOMICILIO : </strong>FUERA DEL PAÍS<br>';
            }
        }
        if (ltrim($data["cpcodcam"], "0") != '') {
            $txt .= '<strong>CAMARA DE COMERCIO : </strong>' . retornarNombreCamaraMysqli2($mysqli, $data["cpcodcam"]) . '<br>';
        }
...
function armarCertificaActividad($pdf, $data, $mysqli = null) {
    ...
    if ($data["ciius"][1] != '' ||
        ...
        if ($data["desactiv"] != '' && ($data["organizacion"] == '01' || $data["organizacion"] == '02' || $data["categoria"] == '2' || $data["categoria"] == '3')) {
            $txt = '<strong>DESCRIPCIÓN DE LA ACTIVIDAD ECONÓMICA : </strong>' . $data["desactiv"];
            $pdf->writeHTML($txt, true, false, true, false, 'J');
            $pdf->Ln();
        }

        $txt = '<strong>ACTIVIDAD PRINCIPAL : </strong>' . $data["ciius"][1] . ' - ' . strtoupper(\funcionesSii2::retornarDescripcionCiiu($mysqli, $data["ciius"][1]));
        if ($data["ciius"][2] != '') {
            $txt .= '<br><strong>ACTIVIDAD SECUNDARIA : </strong>' . $data["ciius"][2] . ' - ' . strtoupper(\funcionesSii2::retornarDescripcionCiiu($mysqli, $data["ciius"][2]));
        }
        if ($data["ciius"][3] != '') {
            $txt .= '<br><strong>OTRAS ACTIVIDADES : </strong>' . $data["ciius"][3] . ' - ' . strtoupper(\funcionesSii2::retornarDescripcionCiiu($mysqli, $data["ciius"][3]));
        }
        if ($data["ciius"][4] != '') {
            $txt .= '<br><strong>OTRAS ACTIVIDADES : </strong>' . $data["ciius"][4] . ' - ' . strtoupper(\funcionesSii2::retornarDescripcionCiiu($mysqli, $data["ciius"][4]));
        }
        $pdf->writeHTML($txt, true, false, true, false, 'L');
        $pdf->Ln();
    } else {
        if (isset($data["ciius3"])) {
            if ($data["ciius3"][1] != '' ||
                ...
                $txt = '<strong>ACTIVIDAD PRINCIPAL : </strong>' . $data["ciius3"][1] . ' ' . strtoupper(\funcionesSii2::retornarDescripcionCiiu($mysqli, $data["ciius3"][1], '3.1'));
                if ($data["ciius3"][2] != '') {
                    $txt .= '<br><strong>ACTIVIDAD SECUNDARIA : </strong>' . $data["ciius3"][2] . ' ' . strtoupper(\funcionesSii2::retornarDescripcionCiiu($mysqli, $data["ciius3"][2], '3.1'));
                }
                if ($data["ciius3"][3] != '') {
                    $txt .= '<br><strong>OTRAS ACTIVIDADES : </strong>' . $data["ciius3"][3] . ' ' . strtoupper(\funcionesSii2::retornarDescripcionCiiu($mysqli, $data["ciius3"][3], '3.1'));
                }
                if ($data["ciius3"][4] != '') {
                    $txt .= '<br><strong>OTRAS ACTIVIDADES : </strong>' . $data["ciius3"][4] . ' ' . strtoupper(\funcionesSii2::retornarDescripcionCiiu($mysqli, $data["ciius3"][4], '3.1'));
                }
                $pdf->writeHTML($txt, true, false, true, false, 'L');
                $pdf->Ln();
...
function armarCertificaTransporte($pdf, $data, $mysqli = null) {
    ...
    //
    if ($data["ciius"][1] == 'H4923' ||
            $data["ciius"][2] == 'H4923' ||
            $data["ciius"][3] == 'H4923' ||
            $data["ciius"][4] == 'H4923') {
        $estransportecarga = 'si';
    }
    if ($data["ciius"][1] == 'H4921' ||
            $data["ciius"][2] == 'H4921' ||
            $data["ciius"][3] == 'H4921' ||
            $data["ciius"][4] == 'H4921') {
                ...
                if ($i["crev"] != '1' && $i["crev"] != '9') {
                    if ($ins["idmunidoc"] != '') {
                        if (trim($ins["txoridoc"]) != '') {
                            $txtmunicipio = 'EN ' . retornarNombreMunicipioMysqli2($mysqli, $ins["idmunidoc"]);
                            $txt = 'MEDIANTE INSCRIPCION NO. ' . $ins["nreg"] . ' DE FECHA ' . strtoupper(\funcionesSii2::mostrarFechaLetras1($ins["freg"])) . ' ';
                            $txt .= 'SE REGISTRO EL ACTO ADMINISTRATIVO NO. ' . $ins["ndoc"] . ' DE FECHA ' . strtoupper(\funcionesSii2::mostrarFechaLetras1($ins["fdoc"])) . ', EXPEDIDO POR ';
                            ...
                            $txtmunicipio = 'EXPEDIDO EN ' . retornarNombreMunicipioMysqli2($mysqli, $ins["idmunidoc"]);
                            $txt = 'MEDIANTE INSCRIPCION NO. ' . $ins["nreg"] . ' DE FECHA ' . strtoupper(\funcionesSii2::mostrarFechaLetras1($ins["freg"])) . ' ';
                            $txt .= 'SE REGISTRO EL ACTO ADMINISTRATIVO NO. ' . $ins["ndoc"] . ' DE FECHA ' . strtoupper(\funcionesSii2::mostrarFechaLetras1($ins["fdoc"])) . ', ' . $txtmuncipio;
                        ...
                        if (trim($ins["txoridoc"]) != '') {
                            ...
                            $txt = 'MEDIANTE INSCRIPCION NO. ' . $ins["nreg"] . ' DE FECHA ' . strtoupper(\funcionesSii2::mostrarFechaLetras1($ins["freg"])) . ' ';
                            $txt .= 'SE REGISTRO EL ACTO ADMINISTRATIVO NO. ' . $ins["ndoc"] . ' DE FECHA ' . strtoupper(\funcionesSii2::mostrarFechaLetras1($ins["fdoc"])) . ', ';
                            $txt .= 'QUE LO HABILITA PARA PRESTAR EL SERVICIO ';
                            $txt .= 'PUBLICO DE TRANSPORTE TERRESTRE AUTOMOTOR EN LA MODALIDAD DE CARGA.';
                        }
                    }
...
function armarCertificaTextoLibreClase($pdf, $data, $clase = '', $titulo = '', $titulos = 'si', $mysqli = null) {
    ...
    if ($clase != 'AC-SOCIOS' && $clase != 'CRT-REFORMAS' && $clase != 'CRT-REFORMAS-HTML' && $clase != 'CRT-VARIOS') {
        ...
        if ($txtx != '') {
            ...
            // 2019-07-06: JINT: SE ACTIVA LA GENERACION DE TEXTOS PASANDO DESDE HTML A PDF
            // $pdf->MultiCell(185, 4, $txtx . chr(10) . chr(13) . chr(10) . chr(13), 0, 'J', 0);
            // $pdf->writeHTML($txtx, true, false, true, false, 'J');
            // 2019-07-18: JINT: SE DESACTIVA LA GENERACION DE TEXTOS PASANDO DESDE HTML A PDF
            $pdf->MultiCell(185, 4, $txtx . chr(10) . chr(13) . chr(10) . chr(13), 0, 'J', 0);
    ...
    // JINT - 2019 09 06 - para incluir certificas en HTML
...
// *************************************************************************** //    
function armarCertificaTextoPropio($pdf, $data, $pantalla = '', $titulo = '', $titulos = 'si', $mysqli = null) {
    $cerrarMysqli = 'no';
    if ($mysqli == null) {
        $cerrarMysqli = 'si';
    ...
    $retornar = true;
    if ($pantalla != '') {
        $texto = \funcionesSii2::cambiarSustitutoHtml(\funcionesSii2::retornarPantallaPredisenada(null, $pantalla));
        if ($texto != '') {
            $pdf->SetFont('courier', '', $pdf->tamanoLetra);
            if ($titulos == 'si') {
                if ($_SESSION["generales"]["TITULOS_EN_CERTIFICADOS_SII"] != 'NO') {
...
function armarCertificaCambioDomicilio($pdf, $data, $mysqli = null) {
    ...
    if (ltrim($data["camant"], "0") != '') {
        ...
        $txt = 'QUE EL COMERCIANTE CAMBIO SU DOMICILIO DESDE ' . retornarNombreMunicipioMysqli2($mysqli, $data["munant"]) . ', ';
        $txt .= 'DONDE ESTUVO MATRICULADO BAJO EL NUMERO ' . $data["matant"] . " DEL " . strtoupper(\funcionesSii2::mostrarFechaLetras1($data["fecmatant"]));
        $pdf->writeHTML('<span style="text-align:justify;">' . $txt . '</span>', true, false, true, false);
        $pdf->Ln();
    }
}
...
function armarCertificaCambiosNombre($pdf, $data, $mysqli = null) {
    ...
    if (!empty($data["nomant"])) {
        ...
        foreach ($data["nomant"] as $n) {
            $nomx = \funcionesSii2::borrarPalabrasAutomaticas($n["nom"]);
            $in++;
        ...
        $nomx = \funcionesSii2::borrarPalabrasAutomaticas($data["nombre"], $data["complementorazonsocial"]);
        $txt .= '<tr align="justify">';
        ...
        foreach ($data["nomant"] as $n) {
            ...
            if (!empty($i)) {
                ...
                if (isset($data["nomant"][$num1])) {
                    $nomx = \funcionesSii2::borrarPalabrasAutomaticas($n["nom"]);
                    $nomantx = \funcionesSii2::borrarPalabrasAutomaticas($data["nomant"][$num1]["nom"]);
                    $txt .= $nomx . ' POR ' . $nomantx;
                } else {
                    $nomx = \funcionesSii2::borrarPalabrasAutomaticas($n["nom"]);
                    $nomantx = \funcionesSii2::borrarPalabrasAutomaticas($data["nombre"], $data["complementorazonsocial"]);
                    $txt .= $nomx . ' POR ' . $nomantx;
                }
                $pdf->writeHTML('<span style="text-align:justify;">' . $txt . '</span>', true, false, true, false);
                $pdf->Ln();
...
function armarCertificaDepuracion1727($pdf, $data, $mysqli = null) {
    ...
    if ($data["ctrdepuracion1727"] == 'S') {
        ...
        if ($data["ctrfechadepuracion1727"] == '') {
            ...
            $txt = 'QUE EN CUMPLIMIENTO DE LO ESTABLECIDO EN EL ARTÍCULO 31 DE LA ';
            $txt .= 'LEY 1727 DE 2014, EL ' . \funcionesSii2::mostrarFechaLetras($data["ctrfechadepuracion1727"]) . ' ';
        }
        if ($data["organizacion"] == '01' ||
                $data["organizacion"] == '02' ||
                $data["categoria"] == '2' ||
...
function armarCertificaEnDepuracion1727($pdf, $data, $mysqli = null) {
    if (date("md") > $_SESSION["generales"]["fcortemesdia"] && date("md") <= substr($_SESSION["generales"]["fcortemesdia"],0,2) . '31') {
        if ($data["estadomatricula"] != 'IC' &&
                $data["estadomatricula"] != 'MC' &&
                $data["estadomatricula"] != 'MF' &&
        ...
        if ($data["estadomatricula"] == 'MF' || $data["estadomatricula"] == 'MC') {
            ...
            if ($cancelacion == 'no') {
                if ($data["fechacancelacion"] != '') {
                    ...
                    $txt = 'LA MATRICULA SE ENCUENTRA CANCELADA EN EL REGISTRO PÚBLICO MERCANTIL A PARTIR DEL ' . \funcionesSii2::mostrarFechaLetras1($data["fechacancelacion"]);
                    $pdf->writeHTML($txt, true, false, true, false, 'J');
                    $pdf->Ln();
                }
            }
...
// *************************************************************************** //
function armarCertificaCancelacion($pdf, $data) {
    //
    $pdf->SetFont('courier', '', $pdf->tamanoLetra);
...
function armarCertificaVigencia($pdf, $data, $mysqli = null) {
    ...
    if ($data["estadomatricula"] == 'MA' || $data["estadomatricula"] == 'MI' ||
            ...
            if ($mostrar == 'si') {
                ...
                if (ltrim(trim($data["fechavencimiento"]), "0") != '') {
                    if (ltrim(trim($data["fechavencimiento"]), "0") != '99999997') {
                        ...
                        $txt .= 'DURACIÓN, QUE SU VIGENCIA FUE HASTA EL ' . strtoupper(\funcionesSii2::mostrarFechaLetras1(trim($data["fechavencimiento"])));
                    } else {
                        $estextual = 'si';
                    }
                } else {
            ...
            if ($data["disueltaporacto510"] == 'si') {
                ...
                if (ltrim(trim($data["fechavencimiento"]), "0") != '') {
                    if ($data["fechavencimiento"] > date("Ymd")) {
                        ...
                        $txt = 'QUE LA PERSONA JURÍDICA SE ENCUENTRA DISUELTA Y EN CAUSAL DE LIQUIDACIÓN. ';
                        $txt .= 'QUE SU VIGENCIA ERA HASTA EL ' . strtoupper(\funcionesSii2::mostrarFechaLetras1(trim($data["fechavencimiento"])));
                    }
                } else {
                    $txt = 'QUE LA PERSONA JURÍDICA SE ENCUENTRA DISUELTA Y EN CAUSAL DE LIQUIDACIÓN. ';
                }
        ...
        if ($data["disueltaporvencimiento"] != 'si' && $data["disueltaporacto510"] != 'si') {
            if (ltrim(trim($data["fechavencimiento"]), "0") == '' || ltrim(trim($data["fechavencimiento"]), "0") == '9999997') {
                ...
                if (ltrim(trim($data["fechavencimiento"]), "0") != '' && ltrim(trim($data["fechavencimiento"]), "0") != '99999997' && ltrim(trim($data["fechavencimiento"]), "0") != '9999998' && ltrim(trim($data["fechavencimiento"]), "0") != '99999999') {
                    ...
                    $txt = 'QUE LA DURACIÓN DE LA PERSONA JURÍDICA (VIGENCIA) ES HASTA EL ' . strtoupper(\funcionesSii2::mostrarFechaLetras1(trim($data["fechavencimiento"])));
                    $pdf->writeHTML('<span style="text-align:justify;">' . $txt . '</span>', true, false, true, false);
                    $pdf->Ln();
                } else {
    ...
    if ($data["estadomatricula"] == 'MC' || $data["estadomatricula"] == 'MF' || $data["estadomatricula"] == 'IC') {
        if ($data["disueltaporvencimiento"] == 'si') {
            ...
            if (ltrim(trim($data["fechavencimiento"]), "0") != '') {
                if (ltrim(trim($data["fechavencimiento"]), "0") != '99999997') {
                    if ($data["fechavencimiento"] <= date("Ymd")) {
                        $txt = 'QUE LA VIGENCIA DE LA PERSONA JURIDICA FUE HASTA EL ' . strtoupper(\funcionesSii2::mostrarFechaLetras(trim($data["fechavencimiento"])));
                    } else {
                        $txt = 'QUE LA VIGENCIA DE LA PERSONA JURIDICA ES HASTA EL ' . strtoupper(\funcionesSii2::mostrarFechaLetras(trim($data["fechavencimiento"])));
                    }
                } else {
                    $estextual = 'si';
                }
        ...
        if ($data["disueltaporvencimiento"] != 'si' && $data["disueltaporacto510"] != 'si') {
            if (ltrim(trim($data["fechavencimiento"]), "0") == '99999997' || ltrim(trim($data["fechavencimiento"]), "0") == '9999997') {
                ...
                if (ltrim(trim($data["fechavencimiento"]), "0") != '' && ltrim(trim($data["fechavencimiento"]), "0") != '99999997' && ltrim(trim($data["fechavencimiento"]), "0") != '9999997' && ltrim(trim($data["fechavencimiento"]), "0") != '99999999') {
                    ...
                    if ($data["fechavencimiento"] <= date("Ymd")) {
                        $txt = 'QUE LA VIGENCIA DE LA PERSONA JURIDICA FUE HASTA EL ' . strtoupper(\funcionesSii2::mostrarFechaLetras1(trim($data["fechavencimiento"])));
                    } else {
                        $txt = 'QUE LA VIGENCIA DE LA PERSONA JURIDICA ES HASTA EL ' . strtoupper(\funcionesSii2::mostrarFechaLetras1(trim($data["fechavencimiento"])));
                    }
                    $pdf->writeHTML('<span style="text-align:justify;">' . $txt . '</span>', true, false, true, false);
                    $pdf->Ln();
                } else {
...
function armarCertificaVigilanciaControl($pdf, $data, $mysqli = null) {
    if ($data["categoria"] == '1') {
        if (ltrim(trim($data["vigcontrol"]), "0") != '') {
            $nom = retornarNombreTablasSirepMysqli2($mysqli, '43', $data["vigcontrol"]);
            if ($nom == '') {
                    ...
                    if (trim($e["numreg"]) != '') {
                        ...
                        foreach ($data["inscripciones"] as $ins) {
                            if (!isset($e["dupli"]) || $e["dupli"] == '') { // Si no está definido el dupli
                                if ($e["libro"] == $ins["lib"] &&
                                        $e["numreg"] == $ins["nreg"]) {
                                    $txMun = retornarNombreMunicipioMysqli2($mysqli, $ins["idmunidoc"]);
                                    $ndocext = $ins["ndocext"];
                                    $ins1 = $ins;
                                }
                            } else {
...
// *************************************************************************** //
function armarCertificaEmbargosEstablecimientosCancelados($pdf, $data, $mysqli) {
    //
    $sitiene = 'no';
    ...
    if ($data["establecimientosh"] && !empty($data["establecimientosh"])) {
        foreach ($data["establecimientosh"] as $h) {
            $embs = retornarRegistrosMysqli2($mysqli, 'mreg_est_embargos', "matricula='" . $h["matriculaestablecimiento"] . "'", "fecinscripcion");
            if ($embs && !empty($embs)) {
                foreach ($embs as $emb) {
                    if ($emb["acto"] == '0900' || $emb["acto"] == '1000') {
                        if ($emb["ctrestadoembargo"] == '1') {
                            ...
                            if ($emb["dupli"] == '') {
                                $insx = retornarRegistroMysqli2($mysqli, 'mreg_est_inscritos', "libro='" . $emb["libro"] . "' and registro='" . $emb["numreg"] . "'");
                            } else {
                                $insx = retornarRegistroMysqli2($mysqli, 'mreg_est_inscritos', "libro='" . $emb["libro"] . "' and registro='" . $emb["numreg"] . "' and dupli='" . $emb["dupli"] . "'");
                            }
                            if ($insx && !empty($insx)) {
                                $txMun = retornarRegistroMysqli2($mysqli, 'bas_municipios', "codigomunicipio='" . $insx["municipiodocumento"] . "'", "ciudad");
    ...
    if ($tieneestablecimientos == 'si') {
        ...
        foreach ($data["establecimientos"] as $e) {
            if ($e["estadodatosestablecimiento"] == 'MA' || $e["estadodatosestablecimiento"] == 'MI') {
                ...
                if (trim($e["barriocom"]) != '') {
                    //  $txt .= '<strong>BARRIO  : </strong>' . retornarNombreBarrioMysqli2($mysqli, $e["muncom"], $e["barriocom"]) . '<br>';
                    $txt .= '<strong>BARRIO  : </strong>' . retornarRegistroMysqli2($mysqli, 'mreg_barriosmuni', "idmunicipio='" . $e["muncom"] . "' and idbarrio='" . $e["barriocom"] . "'", "nombre") . '<br>';
                }

                $txt .= '<strong>MUNICIPIO  : </strong>' . $e["muncom"] . ' - ' . retornarNombreMunicipioMysqli2($mysqli, $e["muncom"]) . '<br>';
                if ($e["telcom1"] != '') {
                ...
                $txt .= '<strong>ACTIVIDAD PRINCIPAL : </strong>' . $e["ciiu1"] . ' - ' . strtoupper(\funcionesSii2::retornarDescripcionCiiu($mysqli, $e["ciiu1"]));
                if ($e["ciiu2"] != '') {
                    $txt .= '<br><strong>ACTIVIDAD SECUNDARIA : </strong>' . $e["ciiu2"] . ' - ' . strtoupper(\funcionesSii2::retornarDescripcionCiiu($mysqli, $e["ciiu2"]));
                }
                if ($e["ciiu3"] != '') {
                    $txt .= '<br><strong>OTRAS ACTIVIDADES : </strong>' . $e["ciiu3"] . ' - ' . strtoupper(\funcionesSii2::retornarDescripcionCiiu($mysqli, $e["ciiu3"]));
                }
                if ($e["ciiu4"] != '') {
                    $txt .= '<br><strong>OTRAS ACTIVIDADES : </strong>' . $e["ciiu4"] . ' - ' . strtoupper(\funcionesSii2::retornarDescripcionCiiu($mysqli, $e["ciiu4"]));
                }
                ...
                foreach ($_SESSION["generales"]["clasecerti"] as $certif => $dtax) {
                    if ($dtax["clase"] == 'CRT-VARIOS') {
                        $exptem["crt"][$certif] = retornarRegistroMysqli2($mysqli, 'mreg_est_certificas', "matricula='" . $e["matriculaestablecimiento"] . "' and idcertifica='" . $certif . "'", "texto");
                        if ($exptem["crt"][$certif] == false || empty($exptem["crt"][$certif])) {
                            $exptem["crt"][$certif] = '';
                        }
                        $exptem["crtsii"][$certif] = retornarRegistroMysqli2($mysqli, 'mreg_certificas_sii', "expediente='" . $e["matriculaestablecimiento"] . "' and idcertifica='" . $certif . "'", "contenido");
    ...
    if ($tienesucage == 'si') {
        ...
        foreach ($data["sucursalesagencias"] as $e) {
            if ($e["estado"] == 'MA' || $e["estado"] == 'MI' || $e["estado"] == 'IA' || $e["estado"] == 'II') {
                ...
                $txt .= '<strong>ACTIVIDAD PRINCIPAL : </strong>' . $e["ciiu1"] . ' - ' . \funcionesSii2::retornarDescripcionCiiu($mysqli, $e["ciiu1"]);
                if ($e["ciiu2"] != '') {
                    $txt .= '<br><strong>ACTIVIDAD SECUNDARIA : </strong>' . $e["ciiu2"] . ' - ' . \funcionesSii2::retornarDescripcionCiiu($mysqli, $e["ciiu2"]);
                }
                if ($e["ciiu3"] != '') {
                    $txt .= '<br><strong>OTRAS ACTIVIDADES : </strong>' . $e["ciiu3"] . ' - ' . \funcionesSii2::retornarDescripcionCiiu($mysqli, $e["ciiu3"]);
                }
                if ($e["ciiu4"] != '') {
                    $txt .= '<br><strong>OTRAS ACTIVIDADES : </strong>' . $e["ciiu4"] . ' - ' . \funcionesSii2::retornarDescripcionCiiu($mysqli, $e["ciiu4"]);
                }
        ...
        if ($insc["grupoacto"] == '003' || $insc["grupoacto"] == '072') {
            ...
            if ($insc["crev"] != '1' && $insc["crev"] != '9') {
                ...
                if ($cert003 == 'si') {
                    ...
                    $txt = 'QUE EL ' . strtoupper(\funcionesSii2::mostrarFechaLetras1($insc["freg"])) . ' SE REGISTRÓ PARA EL ESTABLECIMIENTO ';
    ...
    if ($tienepropietarios == 'si') {
        ...
        foreach ($data["propietarios"] as $e) {
            ...
            if ($e["idtipoidentificacionpropietario"] == '2') {
                $sp = \funcionesSii2::separarDv($e["identificacionpropietario"]);
                $txt .= '<strong>NIT  : </strong>' . $sp["identificacion"] . '-' . $sp["dv"] . '<br>';
            } else {
                $txt .= '<strong>IDENTIFICACIÓN  : </strong>' . retornarNombreTablaBasicaMysqli2($mysqli, 'mreg_tipoidentificacion', $e["idtipoidentificacionpropietario"]) . ' - ' . $e["identificacionpropietario"] . '<br>';
                if (ltrim($e["nitpropietario"], "0") != '') {
                    $sp = \funcionesSii2::separarDv($e["nitpropietario"]);
                    $txt .= '<strong>NIT  : </strong>' . $sp["identificacion"] . '-' . $sp["dv"] . '<br>';
                }
            }
            if ($e["camarapropietario"] == CODIGO_EMPRESA || $e["camarapropietario"] == '') {
                ...
                if (trim($e["direccionpropietario"]) != '') {
                    $txt .= '<strong>MUNICIPIO  : </strong>' . $e["municipiopropietario"] . ' - ' . retornarNombreMunicipioMysqli2($mysqli, $e["municipiopropietario"]) . '<br>';
                }
            }
            $pdf->writeHTML($txt, true, false, true, false, 'L');
            $pdf->Ln();
...
function armarCertificaCae($pdf, $data, $mysqli = null) {
    ...
    //
    $txtCuerpo = retornarRegistroMysqli2($mysqli, 'mreg_alcaldias', "idmunicipio='" . $data["muncom"] . "'", "informa");
    if (trim($txtCuerpo) != '') {
        $txtCuerpo = str_replace("[MUNICIPIO]", retornarNombreMunicipioMysqli2($mysqli, $data["muncom"]), $txtCuerpo);
        $txtCuerpo = str_replace("[FECHAENVIO]", mostrarFechaSii2($data["placaalcaldiafecha"]), $txtCuerpo);
    ...
    $txtCuerpo .= "HACIENDA MUNICIPAL DE LA ALCALDIA DE " . retornarNombreMunicipioMysqli2($mysqli, $data["muncom"]) . " Y BOMBEROS, ";
    ...
    if ($_SESSION["generales"]["codigoempresa"] == '11') {
        if ($data["placaalcaldia"] != '') {
            ...
            $txtCuerpo .= mostrarFechaSii2($data["placaalcaldiafecha"]) . ", PARA IDENTIFICAR ESTE NÚMERO DE MATRÍCULA MERCANTIL.";
        }
    }
    ...
    foreach ($data["vinculos"] as $v) {
        if ($v["vinculootros"] == $tipovinculo) {
            ...
            if (trim($v["cargootros"]) == '') {
                $descCargo = retornarNombreTablasSirepMysqli2($mysqli, '14', $v["idcargootros"]);
                    ...
                    if ($v["vinculootros"] == $tipovinculo) {
                        ...
                        if ($nom == $x) {
                            ...
                            if (trim($v["cargootros"]) == '') {
                                ...
                                $arrCargo = retornarNombreTablasSirepMysqli2($mysqli, '14', $v["idcargootros"]);
...
function descripciones($mysqli, $organizacion, $acto, $tipdoc, $numdoc, $numdocext, $fecdoc, $idorigen, $txtorigen, $munori, $libro, $registro, $fecins, $noticia, $nan = array(), $nombre = '', $comple = '', $camant = '', $libant = '', $regant = '', $fecant = '') {
    ...
    //
    $txtDoc = retornarNombreTablaBasicaMysqli2($mysqli, 'mreg_tipos_documentales_registro', $tipdoc);
    if ($txtDoc == '') {
        $txtDoc = 'DOCUMENTO';
    }
    ...
    if ($fecdoc != '' && $tipdoc != '38') {
        $txt .= 'DEL ' . strtoupper(\funcionesSii2::mostrarFechaLetras1($fecdoc)) . ' ';
    }
    ...
    if ($tipdoc != '38') {
        if ($txtorigen != '') {
            ...
            if (ltrim($idorigen, "0") != '' && ltrim($idorigen, "0") != '999999') {
                $txtSuscribe = retornarNombreTablasSirepMysqli2($mysqli, '12', $idorigen);
            }
        }
        ...
        if ($tipdoc == '02' || $tipdoc == '04') {
            if ($munori != '' && $munori != '00000' && $munori != '99999') {
                $txt .= ' DE ' . retornarNombreMunicipioMysqli2($mysqli, $munori) . ', ';
            } else {
                $txt .= ', ';
            }
        } else {
    ...
    $txt .= 'DEL LIBRO ' . $txtLibro . ' EL ' . strtoupper(\funcionesSii2::mostrarFechaLetras1($fecins)) . ', ';

    if ($camant != '') {
        $txt .= 'INSCRITO ORIGINALMENTE EL ' . strtoupper(\funcionesSii2::mostrarFechaLetras1($fecant)) . ' EN LA ' . retornarNombreCamaraMysqli2($mysqli, $camant);
        if ($regant != '') {
    ...
    if ($acto == '0040') { // Constitución
        if (!empty($nan)) {
            ...
            if ($nombre != '') {
                $nom1 = \funcionesSii2::borrarPalabrasAutomaticas($nombre, $comple);
                if ($tipdoc == '10') {
...
function truncarDecimales($valor, $decimalesVisibles = '2') {
    return number_format(\funcionesSii2::truncateFloat($valor, $decimalesVisibles), $decimalesVisibles, ',', '.');
}

function diferenciaEntreFechaBase30CertificadosSii2($fechafinal, $fechainicial) {
...
function convertirMayusculasSii2($org, $cat) {
    $retornar = false;
