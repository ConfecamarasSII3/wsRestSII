...
trait consultarExpedienteMercantil {
    ...
    public function consultarExpedienteMercantil(API $api) {
        ...
        $_SESSION["jsonsalida"]["estadoproponente"] = '';
        ...
        $_SESSION["jsonsalida"]["pagos"] = array();
        $_SESSION["jsonsalida"]["establecimientos"] = array();
        $_SESSION["jsonsalida"]["vinculos"] = array();
        ...
        $_SESSION["jsonsalida"]["proponente"] = ltrim((string) $arrTem["proponente"], "0");
        ...
        $_SESSION["jsonsalida"]["estado"] = $arrTem["estadoproponente"];
        ...
        //
        $anos = array();
        $pags = retornarRegistrosMysqliApi($mysqli, 'mreg_est_recibos', "matricula='" . $arrTem["matricula"] . "'", "fecoperacion");
        if ($pags && !empty($pags)) {
            foreach ($pags as $pg) {
                if (substr($pg["servicio"], 0, 6) == '010201' ||
                        substr($pg["servicio"], 0, 6) == '010202' ||
                        substr($pg["servicio"], 0, 6) == '060100' ||
                        substr($pg["servicio"], 0, 6) == '010901'
                ) {
                    if ($pg["ctranulacion"] == '0' && substr($pg["numerorecibo"], 0, 1) == 'S') {
                        $row = array();
                        $row["numerorecibo"] = $pg["numerorecibo"];
                        $row["fecoperacion"] = $pg["fecoperacion"];
                        $row["fecharenovacionaplicable"] = $pg["fecharenovacionaplicable"];
                        $row["ctranulacion"] = $pg["ctranulacion"];
                        $row["servicio"] = $pg["servicio"];
                        $row["nombreservicio"] = retornarRegistroMysqliApi($mysqli, 'mreg_servicios', "idservicio='" . $pg["servicio"] . "'", "nombre");
                        $row["valor"] = $pg["valor"];
                        $row["anorenovacion"] = trim((string) $pg["anorenovacion"]);
                        $row["reliquidacion"] = '';
                        if (substr($pg["servicio"], 0, 6) == '010202') {
                            if (isset($anos[$pg["anorenovacion"]])) {
                                $row["reliquidacion"] = 'si';
                            }
                            $anos[$pg["anorenovacion"]] = $pg["anorenovacion"];
                        }
                        $_SESSION["jsonsalida"]["pagos"][] = $row;
                    }
                }
            }
        }
        ...
        if ($arrTem["organizacion"] == '01' || ($arrTem["organizacion"] > '02' && $arrTem["categoria"] == '1')) {
            ...
            $arrGrupoVin = array();
            $arrVincs = array();
            foreach ($arrTem["vinculos"] as $vin) {
                if (!isset($arrTipoIde[$vin["idtipoidentificacionotros"]])) {
                    $arrTipoIde[$vin["idtipoidentificacionotros"]] = retornarRegistroMysqliApi($mysqli, 'mreg_tipoidentificacion', "id='" . $vin["idtipoidentificacionotros"] . "'", "descripcion");
                }
                if (!isset($arrGrupoVin[$vin["tipovinculo"]])) {
                    $arrGrupoVin[$vin["tipovinculo"]] = retornarRegistroMysqliApi($mysqli, 'mreg_clasevinculos', "id='" . $vin["tipovinculo"] . "'", "descripcion");
                }
                if (!isset($arrVincs[$vin["vinculootros"]])) {
                    $arrVincs[$vin["vinculootros"]] = retornarRegistroMysqliApi($mysqli, 'mreg_codvinculos', "id='" . $vin["vinculootros"] . "'", "descripcion");
                }

                $iVin = array();
                $iVin["idclase"] = $vin["idtipoidentificacionotros"];
                $iVin["descripcionidclase"] = $arrTipoIde[$vin["idtipoidentificacionotros"]];
                $iVin["numid"] = $vin["identificacionotros"];
                $iVin["nombre"] = $vin["nombreotros"];
                $iVin["vinculo"] = $vin["vinculootros"];
                $iVin["descripcionvinculo"] = $arrVincs[$vin["vinculootros"]];
                $iVin["cargo"] = $vin["cargootros"];
                $iVin["tipovinculo"] = $vin["tipovinculo"];
                $iVin["descripciontipovinculo"] = $arrGrupoVin[$vin["tipovinculo"]];
                $iVin["fecharegistro"] = $vin["fechaotros"];
                $iVin["inscripcion"] = $vin["librootros"] . '-' . $vin["inscripcionotros"] . '-' . $vin["dupliotros"];
                $iVin["tarjetaprofesional"] = $vin["numtarprofotros"];
                $_SESSION["jsonsalida"]["vinculos"][] = $iVin;
            }
        }
