...
trait consultarExpedienteProponente
{
    ...
    public function consultarExpedienteProponente(API $api)
    {
        ...
        if (trim((string)$_SESSION["entrada"]["proponente"]) == '' && trim((string)$_SESSION["entrada"]["identificacion"]) == '') {
            $_SESSION["jsonsalida"]["codigoerror"] = "9999";
        ...
        if ($arrTemRup) {
            ...
            $temx = retornarRegistroMysqliApi($mysqli, 'bas_gruponiif', "id='" . $arrTemRup["gruponiif"] . "'");
            if ($temx && !empty($temx)) {
            ...
            if (isset($arrRepLegal["representanteslegales"]) && !empty($arrRepLegal["representanteslegales"])) {
