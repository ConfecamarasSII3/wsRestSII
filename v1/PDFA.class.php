...
class PDFA {
    ...
    public function generarPDFAfirmado($aleatorio, $rutaPDFRepositorioBase, $convertirPDFA = 'si', $tamanioPagina = 'LETTER') {
        ...
        $rutaOutPDFA = limpiarPath($_SESSION["generales"]["pathabsoluto"] . '/tmp/' . $aleatorio . '_tmp.pdf');
        $rutaOutFirmado = limpiarPath($_SESSION["generales"]["pathabsoluto"] . '/tmp/' . $aleatorio . '_firmado.pdf');
        ...
        if ($convertirPDFA == 'si') {
            ...
            if (file_exists($rutaOutPDFA)) {
                ...
                \logApi::general2(__FUNCTION__, $aleatorio, 'Peso Archivo sin firmar:' . FileSizeConvert($peso));
        ...
        if (file_exists($rutaOutFirmado)) {
            ...
            \logApi::general2(__FUNCTION__, $aleatorio, 'Peso Archivo Firmado:' . FileSizeConvert($peso));
        ...
        if (defined($usuarioFirmante)) {
            ...
            if ($usuarioFirmante == 'CERTITOKEN_' . $_SESSION["generales"]["codigoempresa"] . '_0') {
                ...
                if (empty($rutaIDFirma)) {
                    ...
                    log::general2(__FUNCTION__, $usuarioFirmante, $_SESSION["codigoerror"]);
                    return false;
                }
            } else {
    ...
    function firmarPDF($aleatorio = '', $rutaInPDF = '', $rutaOutPdfFirmado = '', $usuarioFirmante = '', $lx = '', $ly = '', $rx = '', $ry = '', $nivelCertificacion = 3) {
        ...
        if (file_exists($rutaInPDF)) {
            ...
            if (defined($usuarioFirmante)) {
                ...
                \logApi::general2('generarPDFAfirmado', $aleatorio, 'JAVA:' . $comandoJava);
                if (!$resultado) {
    ...
    function generarSobreFirmado($hashFirmado, $firmantes = array(), $encabezadoCamara = array(), $detalleTramite = array(), $archivosAdjuntos = array(), $rutaOutPDFA = '', $sobreLink = 'no') {
        ...
        $firmante = utf8_decode($detalleTramite['nomcliente'] . ' - ' . $detalleTramite['nomfirmante']);
        ...
        if (defined($usuarioFirmante)) {
            ...
            $parametros .= "'" . $detalleTramite['idecliente'] . " - " . \EncodingNew::fixUTF8(str_replace("'", "", $detalleTramite['nomcliente'])) . "' ";
            $parametros .= "'" . $detalleTramite['idefirmante'] . " - " . \EncodingNew::fixUTF8(str_replace("'", "", $detalleTramite['nomfirmante'])) . "' ";
            $parametros .= "'" . $detalleTramite['tipotramite'] . "' ";
            $parametros .= "'" . $detalleTramite['numfolios'] . "' ";
            $parametros .= "'" . \EncodingNew::fixUTF8($detalleTramite['dependencia']) . "' ";
            ...
            foreach ($firmantes as $txtFirmante) {
                $parametros .= \EncodingNew::fixUTF8($txtFirmante) . "|";
            }
            $parametros .= "' ";
...
// log::general2(__FUNCTION__, $detalleTramite['idliquidacion'], $comandoJava);
    ...
    function generarSobreSinFirma($encabezadoCamara = array(), $detalleTramite = '', $archivosAdjuntos = array(), $rutaOutPDFA = '') {
        ...
        $detalleSobre = utf8_decode($detalleTramite);
