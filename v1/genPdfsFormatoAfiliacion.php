 ...
 * @return type
 */
function armarPdfFormatoAfiliacionSii($dbx,$numrec, $numliq, $txtFirmaElectronica = '', $txtFirmaManuscrita = '', $nombreFirmante = '', $numIdFirmante = '') {
    ...
    if (!class_exists('PDFCerti')) {
        ...
        class PDFCerti extends TCPDF {
            ...
            function Header() {
                ...
                $this->SetFont('Helvetica', 'B', 10);
                $this->SetXY(20, $i);
                $this->Cell(100, 4, retornarRegistroMysqli2($dbx,'bas_municipios',"codigomunicipio='" . MUNICIPIO . "'","ciudad") . ', ' . mostrarFechaLetrasSii2(date("Ymd")), 0, 0, 'L');
                $this->Ln();
                $this->Ln();
                $i = $this->GetY();
                $this->SetFont('Helvetica', 'B', 9);
                $this->SetXY(20, $i);
                $this->Cell(100, 4, 'Ref. FORMTATO RENOVACION AFILIACION.', 0, 0, 'L');
                $this->Ln();
                $i = $this->GetY();
                $this->SetFont('Helvetica', 'B', 9);
                $this->SetXY(20, $i);
    ...
    $tx .= '<strong>Fecha de matrícula :</strong> ' . mostrarFecha($_SESSION["formulario"]["datos"]["fechamatricula"]) . '<br>';
    ...
    $tx .= 'c) Manifiesto que no he ejecutado ningún acto de comptencia desleal, entendida dicha competencia desleal como todo acto o hecho que se realice en el mercado con fines concurrenciales.¿, cuando resulte contrario a las sanas costumbres mercantiles, al principio de la buena fe comercial, a los usos honestos en materia industrial o comercial, o bien cuando esté encaminado a afectar o afecte la libre decisión del comprador o consumidor, o el fun cionamiento concurrencial del mercado.<br><br>';
