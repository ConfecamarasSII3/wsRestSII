 ...
 * @return type
 */
function armarPdfMutacionGeneralTcpdf($dbx, $numrec, $numliq, $txtFirmaElectronica = '', $txtFirmaManuscrita = '', $idClaseFirmante = '', $numIdFirmante = '', $nombreFirmante = '') {
    ...
    if ($_SESSION["formulario"]["datos"]["organizacion"] == '01') {
        $tx = 'Yo, ' . $_SESSION["formulario"]["datos"]["nombre"] . ' identificado con ' .
                retornarRegistroMysqli2($dbx,'mreg_tipoidentificacion',"id='" . $_SESSION["formulario"]["datos"]["tipoidentificacion"] . "'","descripcion") .
    ...
    if ($_SESSION["formulario"]["datos"]["organizacion"] == '02') {
        if (trim($_SESSION["formulario"]["datos"]["propietarios"][1]["nombrepropietario"]) == '') {
            ...
            $txttipo = retornarRegistroMysqli2($dbx,'mreg_tipoidentificacion',"id='" . $_SESSION["formulario"]["datos"]["propietarios"][1]["idtipoidentificacionpropietario"] . "'","descripcion");
        }
    ...
    if (($_SESSION["formulario"]["datos"]["organizacion"] > '02') && ($_SESSION["formulario"]["datos"]["categoria"] == '1')) {
        if ($txtFirmaElectronica == "") {
            ...
            $txttipo = retornarRegistroMysqli2($dbx,'mreg_tipoidentificacion',"id='" . $_SESSION["tramite"]["tipoidefirmante"] . "'","descripcion");
    ...
    if (($_SESSION["formulario"]["datos"]["organizacion"] > '02') && ($_SESSION["formulario"]["datos"]["categoria"] == '3')) {
        if (
            ...
            $txttipo = retornarRegistroMysqli2($dbx,'mreg_tipoidentificacion',"id='" . $_SESSION["formulario"]["datos"]["idtipoidentificacionadministrador"] . "'","descripcion");
        }
        $tx = 'Yo, ' . $_SESSION["formulario"]["datos"]["nombreadministrador"] . ' identificado con ' .
                ...
                ' matriculada bajo el número ' . $_SESSION["formulario"]["datos"]["matricula"] .
                ' atentamente solicito se apliquen los siguientes cambios al registro :<br><br>';
    }
    ...
    if ($_SESSION["formulario"]["datosanteriores"]["ctrubi"] != $_SESSION["formulario"]["datos"]["ctrubi"]) {
        $tubiant = retornarRegistroMysqli2($dbx,'mreg_ubicacion',"id='" . $_SESSION["formulario"]["datosanteriores"]["ctrubi"] . "'","descripcion");
        $tubi = retornarRegistroMysqli2($dbx,'mreg_ubicacion',"id='" . $_SESSION["formulario"]["datos"]["ctrubi"] . "'","descripcion");
    ...
    $name = session_id() . '-MutacionGeneral-' . date("Ymd") . '-' . date("His") . '.pdf';
