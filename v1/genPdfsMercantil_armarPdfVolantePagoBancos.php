...
function armarPdfVolantePagoBancos($dbx,$liq, $liqdet) {
    ...
    if (defined('CODIGO_IAC')) {
        ...
        if (date("Ymd") <= $fcorte) {
            generarGs1128(trim(CODIGO_IAC), $ref, $fcorte, $imagen);
        } else {
    ...
    $pdf->SetXY(30, 21);
    $pdf->Write(4, "NIT. " . NIT);
    // $pdf->SetFont('Arial','',6);$pdf->SetXY(60,22);$pdf->Write(4,"Recuerde diligenciar las casillas que se encuentran en color gris");  
    // Imprime Cuadricula
    ...
    $pdf->SetFont('Arial', 'B', 7);
    $pdf->SetXY(60, 61);
    $pdf->Write(5, mostrarFechaSii2(date("Ymd")));
    $pdf->SetFont('Arial', 'B', 7);
    $pdf->SetXY(20, 65);
    $pdf->Write(5, "HORA DE LA LIQUIDACION:");
    $pdf->SetFont('Arial', 'B', 7);
    $pdf->SetXY(60, 65);
    $pdf->Write(5, mostrarHoraSii2(date("His")));
    $pdf->SetFont('Arial', 'B', 7);
    $pdf->SetXY(20, 71);
    ...
    $total = 0;
    foreach ($relServ as $ser) {
        $nomserv = retornarRegistroMysqli2($dbx, 'mreg_servicios', "idservicio='" . $ser["idservicio"] . "'","nombre");
...
function armarPdfVolantePagoBancosOccidente($dbx,$liq, $liqdet, $valliq = 0) {
    ...
    //
    $codigorecaudoraiz = retornarClaveValorSii2($dbx,'90.33.49');
    $codigorecaudo = retornarClaveValorSii2($dbx,'90.33.50');
    ...
    /*
      ...
      $pdf->SetFont('Arial','B',9);$pdf->SetXY(80,29);$pdf->Write(5,"CODIGO PAGO BALOTO: " . $codigorecaudo);
      }
     */
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->SetXY(150, 29);
    ...
    /*
      if (trim($codigorecaudo)!='') {
      $pdf->SetFont('Arial','B',9);$pdf->SetXY(80,138);$pdf->Write(5,"CODIGO PAGO BALOTO: " . $codigorecaudo);
      }
     */
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->SetXY(150, 138);
    ...
    // $txt=RAZONSOCIAL.' - NIT: '.NIT.' - DIRECCION: '.DIRECCION1. ' - TELEFONO: '.PBX;
    // $pdf->SetFont('Arial','',5);$pdf->SetXY(10,249);$pdf->Write(4,$txt);
...
function armarPdfVolantePagoBancosCucuta($dbx,$liq, $liqdet, $valliq = 0) {
    ...
    if (defined('CODIGO_IAC')) {
        $imagen = '../../../tmp/codbarras-' . session_id() . date("Ymd") . date("His") . '.png';
        if (date("Ymd") <= $fcorte) {
            generarGs1128(trim(CODIGO_IAC), $_SESSION["generales"]["codigoempresa"] . sprintf("%011s", $ref), $fcorte, $imagen, $valliq);
        } else {
 ...
 */
function armarPdfVolantePagoBancosCucutaNuevo($dbx,$liq, $liqdet, $valliq = 0, $tipo = 'justificado', $titulo = 'bancos', $textoBancos = '', $textoSoportes = '') {
    require_once ('fpdf153/fpdf.php');
    require_once ('fpdf153/fpdf_code128.php');
    require_once ('generaGS1128.php');
    ...
    if (trim($codigorecaudo) != '') {
        ...
        $pdf->SetFont('Arial', '', 7);
        $pdf->SetXY(116, 45);
        $pdf->Write(5, utf8_decode("Diligenciar este número de manera exacta en los puntos Baloto, "));
        $pdf->SetXY(116, 48);
        $pdf->Write(5, utf8_decode("de lo contrario el pago no se asentará correctamente"));
        $pdf->SetXY(116, 49);
    ...
    if ($liq["tipotramite"] == 'matriculapnat' ||
        ...
        if (defined('CODIGO_BARRAS_MATRICULA')) {
            if (trim(CODIGO_BARRAS_MATRICULA) != '') {
                $imagen = '../../../tmp/codbarras-' . session_id() . date("Ymd") . date("His") . '.png';
                if (date("Ymd") <= $fcorte) {
                    generarGs1128(trim(CODIGO_BARRAS_MATRICULA), $_SESSION["generales"]["codigoempresa"] . sprintf("%011s", $ref), $fcorte, $imagen, $valliq);
                } else {
    ...
    if ($liq["tipotramite"] == 'renovacionmatricula' ||
        ...
        if (defined('CODIGO_BARRAS_RENOVACION')) {
            if (trim(CODIGO_BARRAS_RENOVACION) != '') {
                $imagen = '../../../tmp/codbarras-' . session_id() . date("Ymd") . date("His") . '.png';
                if (date("Ymd") <= $fcorte) {
                    generarGs1128(trim(CODIGO_BARRAS_RENOVACION), $_SESSION["generales"]["codigoempresa"] . sprintf("%011s", $ref), $fcorte, $imagen, $valliq);
                } else {
    ...
    if ($liq["tipotramite"] == 'inscripcionproponente' ||
        ...
        if (defined('CODIGO_BARRAS_PROPONENTES')) {
            if (trim(CODIGO_BARRAS_PROPONENTES) != '') {
                $imagen = '../../../tmp/codbarras-' . session_id() . date("Ymd") . date("His") . '.png';
                if (date("Ymd") <= $fcorte) {
                    generarGs1128(trim(CODIGO_BARRAS_PROPONENTES), $_SESSION["generales"]["codigoempresa"] . sprintf("%011s", $ref), $fcorte, $imagen, $valliq);
                } else {
...
function armarPdfVolantePagoBaloto($dbx,$liq, $liqdet, $valliq = 0, $aleatorio) {
    require_once ('fpdf153/fpdf.php');
    require_once ('fpdf153/fpdf_code128.php');
    require_once ('generaGS1128.php');
...
function armarPdfVolantePagoExito($dbx,$liq, $liqdet, $valliq = 0, $aleatorio) {
    require_once ('fpdf153/fpdf.php');
    require_once ('fpdf153/fpdf_code128.php');
    require_once ('generaGS1128.php');
