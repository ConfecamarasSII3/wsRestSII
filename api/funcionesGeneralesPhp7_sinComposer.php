...
class funcionesGenerales {
     ...
     */
    public static function armarMensaje($txt = '', $javascriptHeaders = '') {
        ...
        if (isset($_SESSION["generales"]["mostrarencabezados"]) && $_SESSION["generales"]["mostrarencabezados"] == 'si') {
            $string .= $pres->armarEncabezado(NOMBRE_SISTEMA, 'Alerta', 'si');
        } else {
            $string .= $pres->armarEncabezado(NOMBRE_SISTEMA, 'Alerta', 'no');
        }
        $string .= '<br>';
    ...
    public static function armarMensajeSimple($txt = '', $path = '') {
        ...
        unset($nw);
        \funcionesGenerales::mostrarCuerpoBootstrapSimple('', '', '', '', '', $string);
        exit();
    }
...
//
    ...
    public static function generarAleatorioAlfanumerico10($dbx, $validar = '', $grabar = 'no') {
        $numrecvalido = 'NO';
        while ($numrecvalido == 'NO') {
            $ok = 'NO';
            while ($ok == 'NO') {
    ...
    public static function retornarSelectTipoDocumental($dbx, $id = '', $modgra = '') {
        ...
        foreach ($lisDoc as $doc) {
            if ($doc["idtipodoc"] == $id) {
                $retornar .= '<option value=' . $doc["idtipodoc"] . ' selected>' . $doc["idtipodoc"] . ' - ' . substr($doc["nombre"], 0, 60) . '</option>';
            } else {
                $retornar .= '<option value=' . $doc["idtipodoc"] . '>' . $doc["idtipodoc"] . ' - ' . substr($doc["nombre"], 0, 60) . '</option>';
            }
        }
        unset($lisDoc);
        unset($doc);
    ...
    //
    public static function retornarArregloLibrosReigstrar($dbx) {
        $sal = array();
        $sal["no"] = "No registrar libro";
    ...
    public static function redondear1000($val) {
        ...
        if ($val2 >= "500") {
            $val = intval($val2) * 1000 + 1000;
        } else {
            $val = intval($val2) * 1000;
        }
        return $val;
    }
    ...
    public static function recuperarImagenRepositorio($img, $sistema = '', $paginas = '', $inicial = '') {
        ...
        if ($sistema == 'DOCUWARE') {
            ...
            if (TIPO_REPOSITORIO_NUEVAS_IMAGENES == 'EFS_S3') {
                ...
                while ($pags <= $paginas && $intentos < 100) {
                    ...
                    $img = recuperarS3Version4Api($fx);
                    if ($img && $img != '' && file_exists($img)) {
                        $fxpdf = $_SESSION["generales"]["pathabsoluto"] . '/tmp/' . $_SESSION["generales"]["codigoempresa"] . '-' . \funcionesGenerales::generarAleatorioAlfanumerico20() . '.pdf';
                        $arrFiles[] = $fxpdf;
                ...
                $filex = $_SESSION["generales"]["pathabsoluto"] . '/tmp/' . $_SESSION["generales"]["codigoempresa"] . '-' . \funcionesGenerales::generarAleatorioAlfanumerico20() . '.pdf';
                unirPdfsApi($arrFiles, $filex, "P", "mm", "Legal");
                return $filex;
            }
        }
        ...
        // Si el repositorio es Amazon Aws S3
        if (TIPO_REPOSITORIO_NUEVAS_IMAGENES == 'S3-V4' || TIPO_REPOSITORIO_NUEVAS_IMAGENES == 'EFS_S3') {
            if (existenciaS3Version4_2Api($img)) {
                $retornar = recuperarS3Version4Api($img);
            } else {
                if (file_exists($_SESSION["generales"]["pathabsoluto"] . '/' . PATH_RELATIVO_IMAGES . '/' . $_SESSION["generales"]["codigoempresa"] . '/' . $img)) {
                    $retornar = $_SESSION["generales"]["pathabsoluto"] . '/' . PATH_RELATIVO_IMAGES . '/' . $_SESSION["generales"]["codigoempresa"] . '/' . $img;
                    $_SESSION["generales"]["mensajeerror"] = '';
