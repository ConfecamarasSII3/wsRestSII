...
trait consultarEstadoRadicado {
    ...
    public function consultarEstadoRadicado(API $api) {
        ...
        //
        $req = retornarRegistroMysqliApi($mysqli, 'mreg_est_codigosbarras', "codigobarras='" . ltrim(trim((string)$_SESSION["entrada"]["radicado"]), "0") . "'");
        if ($req && !empty($req)) {
            ...
            if ($req["estadofinal"] == '05' || $req["estadofinal"] == '06' || $req["estadofinal"] == '07') {
                ...
                $arrTemDev = retornarRegistrosMysqliApi($mysqli, 'mreg_devoluciones_nueva', "idradicacion='" . ltrim(trim((string)$_SESSION["entrada"]["radicado"]), "0") . "'", "fechadevolucion asc, horadevolucion asc");
                if (!empty($arrTemDev)) {
