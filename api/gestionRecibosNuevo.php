...
class gestionRecibosNuevo {
     ...
     * @param type $cajero
     * @param type $requieresoportesasentamiento
     * @return string
     */
    public static function asentarRecibosNuevo($mysqli, $idSolicitudPago, $tipogasto = '0', $estadofinalliquidacion = '09', $fechareciboagenerar = '', $fecharenovacionagenerar = '', $cajero = '', $requieresoportesasentamiento = '') {
        ...
        if ($mysqli === null) {
            $resultado = array(
                'codigoError' => '9999',
                'msgError' => 'No se encontró conexión a la BD activa'
            );
            return $resultado;
        }
        ...
        require_once($_SESSION["generales"]["pathabsoluto"] . '/api/funcionesRegistralesCalculosGestionRecibos.php');
        ...
        $fecharenovacion = ($fecharenovacionagenerar != '') ? $fecharenovacionagenerar : date("Ymd");
        $fecharenaplicable = ($fecharenovacionagenerar != '') ? $fecharenovacionagenerar : $fecharenovacion;
        ...
        // ***************************************************************************************** //    
        // recupera usuario jefe juridico
        // ***************************************************************************************** //
        ...
        // ************************************************************************ //
        if ($cajero != 'USUPUBXX' && $cajero != 'RUE') {
            $usuCajero = retornarRegistroMysqliApi($mysqli, 'usuarios', "idusuario='" . $cajero . "'");
        ...
        // Valida que el estado permite continuar
        // ***************************************************************************************** //
        $datat = \funcionesRegistrales::retornarMregLiquidacion($mysqli, $idSolicitudPago, 'L');
        if ($datat === false || empty($datat)) {
            $resultado["codigoError"] = '9999';
            $resultado["msgError"] = 'Liquidación no encontrada en el sistema de registro';
            $_SESSION["generales"]["codigousuario"] = $_SESSION["generales"]["codigousuariooriginal"];
            return $resultado;
        }
        $datat["tipogasto"] = $tipogasto;
        $datat["cajero"] = $cajero;
        $datat["escajero"] = $escajero;
        $datat["origen"] = $origen;
        if ($datat["sede"] == '') {
            $datat["sede"] = $usuCajero["idsede"];
        }
        if (!isset($_SESSION["generales"]["emailusuariocontrol"]) || $_SESSION["generales"]["emailusuariocontrol"] == '') {
            $_SESSION["generales"]["emailusuariocontrol"] = $datat["emailcontrol"];
        }

        if ($datat["cajero"] == 'USUPUBXX' || $usuCajero["esbanco"] == 'SI') {
            if ($datat["idestado"] > '06' && $datat["idestado"] != '08' && $datat["idestado"] != '19' && $datat["idestado"] != '20' && $datat["idestado"] != '21' && $datat["idestado"] != '44') {
                $resultado["codigoError"] = '9999';
        ...
        //
        if ($datat["cajero"] != 'USUPUBXX' && $usuCajero["esbanco"] != 'SI') {
            if ($datat["idestado"] > '06' && $datat["idestado"] != '19' && $datat["idestado"] != '44' && $datat["idestado"] != '77') {
                if ($datat["idestado"] != '20' || ($datat["idestado"] == '20' && $estadofinalliquidacion != '20')) {
                    if ($datat["idestado"] != '21' || ($datat["idestado"] == '21' && $estadofinalliquidacion != '21')) {
                        $resultado["codigoError"] = '9999';
        ...
        // ************************************************************************************ //
        // Localiza el tipo de Transaccion
        // ************************************************************************************ //
        if ($datat["subtipotramite"] != '' && $datat["tipotramite"] == 'inscripciondocumentos') {
            $arrTipoTramite = retornarRegistroMysqliApi($mysqli, 'bas_tipotramites', "id='" . $datat["subtipotramite"] . "'");
            if ($arrTipoTramite === false || empty($arrTipoTramite) || $arrTipoTramite["gruposervicios"] == '') {
                $resultado["codigoError"] = '9999';
                $resultado["msgError"] = 'No fue posible localizar el sub-tipo de tramite : ' . $datat["subtipotramite"];
                \logApi::general2($nameLog, $idSolicitudPago, $resultado["msgError"]);
                $_SESSION["generales"]["codigousuario"] = $_SESSION["generales"]["codigousuariooriginal"];
                return $resultado;
            }
        } else {
            $arrTipoTramite = retornarRegistroMysqliApi($mysqli, 'bas_tipotramites', "id='" . $datat["tipotramite"] . "'");
            if ($arrTipoTramite === false || empty($arrTipoTramite) || $arrTipoTramite["gruposervicios"] == '') {
                $resultado["codigoError"] = '9999';
                $resultado["msgError"] = 'No fue posible localizar el tipo de tramite : ' . $datat["tipotramite"];
                \logApi::general2($nameLog, $idSolicitudPago, $resultado["msgError"]);
                $_SESSION["generales"]["codigousuario"] = $_SESSION["generales"]["codigousuariooriginal"];
                return $resultado;
            }
        ...
        // ************************************************************************************************************** //
        $generarSII = \funcionesGestionRecibos::validarLiquidacionRecibo($mysqli, $datat["idliquidacion"], $nameLog);
        if ($generarSII["generar"] == 'no') {
            $resultado["codigoError"] = '9999';
            $resultado["msgError"] = 'Recibo de caja generado previamente en SII : Recibo : ' . $generarSII["recibo"] . ', Operacion : ' . $generarSII["operacion"] . ', Fecha y hora : ' . $generarSII["fecha"] . ' ' . $generarSII["hora"] . ', Codigo barras: ' . $generarSII["codigobarras"];
            return $resultado;
        }
        ...
        // ************************************************************************
        $datat["fecharecibogenerar"] = $fechareciboagenerar;
        $datat["fecharenaplicable"] = $fecharenaplicable;
        $datat["fecharenovacion"] = $fecharenovacion;
        $datat["totalcamara"] = 0;
        $datat["totalcamara1"] = 0;
        $datat["totalgobernacion"] = 0;
        $datat["totalgobernacion1"] = 0;
        $datat["tipogasto"] = $tipogasto;
        $datat["tiporegistro"] = $arrTipoTramite["tiporegistro"];        
        $datat["facturableelectronicamente"] = '';
        $datat["tipotramiterue"] = '';
        if (substr($datat["tipotramite"], 0, 4) == 'rues') {
            $datat["tipotramiterue"] = substr($_SESSION["tramite"]["tipotramite"], 6);
        }

        //
        foreach ($datat["liquidacion"] as $l) {
            if (!isset($arrServs[$l["idservicio"]])) {
                ...
                if ($arrServs[$l["idservicio"]]["facturable_electronicamente"] == 'SI') {
                    $datat["facturableelectronicamente"] = 'si';
                }
            }
            if (ltrim((string) $arrServs[$l["idservicio"]]["conceptodepartamental"], "0") != '') {
                $datat["totalgobernacion1"] = $datat["totalgobernacion1"] + $l["valorservicio"];
            } else {
                $datat["totalcamara1"] = $datat["totalcamara1"] + $l["valorservicio"];
            }
        }

        //
        if (!defined('SEPARAR_RECIBOS') || SEPARAR_RECIBOS == '' || SEPARAR_RECIBOS == 'NO') {
            $datat["totalcamara"] = $datat["valortotal"];
        } else {
            foreach ($datat["liquidacion"] as $l) {
                if (isset($arrServs[$l["idservicio"]])) {
                    $arrServs[$l["idservicio"]] = retornarRegistroMysqliApi($mysqli, 'mreg_servicios', "idservicio='" . $l["idservicio"] . "'");
                }
                if (substr($datat["tipotramite"], 6) != 'responsable') {
                    if (ltrim((string) $arrServs[$l["idservicio"]]["conceptodepartamental"], "0") != '') {
                        $datat["totalgobernacion"] = $datat["totalgobernacion"] + $l["valorservicio"];
                    } else {
                        $datat["totalcamara"] = $datat["totalcamara"] + $l["valorservicio"];
                    }
                } else {
                    $datat["totalcamara"] = $datat["totalcamara"] + $l["valorservicio"];
                }
        ...
        // ************************************************************************************************************** //
        $generacionRecibo = \funcionesGestionRecibos::generarRecibo($mysqli, $datat, $nameLog, $arrServs);

        // ************************************************************************************************************** //
        // Si la respuesta es falsa por error en la generacion del recibo de caja
        // ************************************************************************************************************** //
        if ($generacionRecibo === false || $generacionRecibo["codigoError"] != '9999') {
            $resultado["codigoError"] = '9999';
            $resultado["msgError"] = 'No fue posible registrar el recibo en el sistema de Registro : ' . $generacionRecibo["msgError"];
            $txtx = 'No fue posible registrar el recibo en el sistema (3) : ' . $generacionRecibo["msgError"] . ', Error : ' . $_SESSION["generales"]["mensajeerror"];
        ...
        $datat["numerorecibo"] = $generacionRecibo["recibo"];
        $datat["numerorecibogob"] = $generacionRecibo["recibogob"];
        $datat["numerooperacion"] = $generacionRecibo["operacion"];
        $datat["numerooperaciongob"] = $generacionRecibo["operaciongob"];
        $datat["fecharecibo"] = $generacionRecibo["fecha"];
        $datat["fecharecibogob"] = $generacionRecibo["fechagob"];
        $datat["horarecibo"] = $generacionRecibo["hora"];
        $datat["horarecibogob"] = $generacionRecibo["horagob"];
        $datat["totalrecibo"] = $_SESSION["tramite"]["totalcamara"];
        $datat["totalrecibogob"] = $_SESSION["tramite"]["totalgobernacion"];
        \funcionesRegistrales::grabarLiquidacionMreg($mysqli, $datat);
        ...
        $txtx = 'Tipo tramite : ' . $_SESSION["tramite"]["tipotramite"] .
                ', Sub-Tipo tramite : ' . $_SESSION["tramite"]["subtipotramite"] .
        ...
        if ($asignaMatricula == 'si' && $registroInmediato == 'si') {
            ...
            if (
                ...
                if ($tieneformulariosgrabados == 'si') {
                    ...
                    // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Entra a procesar numeros de matricula en SII [' . $pedirPnat . '][' . $pedirEst . '][' . $pedirPjur . '][' . $pedirEsadl . '][' . $pedirSuc . '][' . $pedirAge . ']'));
                    if ($pedirPnat == 'S') {
        ...
        // ************************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Grupo de Servicios : ' . $grupoServicios));
        ...
        if ($incluyePagoAfiliacion == 'si') {
            if ($_SESSION["tramite"]["incrementocupocertificados"] != 0) {
                ...
                // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('asigno/incrmento el cupo de certificados por pago de afiliacion en ' . $cupoAfiliado));
            }
        }
        ...
        if ($generarSII == 'si') {
            ...
            $operacionSII = \funcionesRegistrales::generarSecuenciaOperacion($mysqli, $_SESSION["generales"]["codigousuario"], $fechaSII, $_SESSION["generales"]["cajero"], $usuCajero["idsede"]);
            if ($totalgobernacion != 0) {
                $operacionSIIgob = \funcionesRegistrales::generarSecuenciaOperacion($mysqli, $_SESSION["generales"]["codigousuario"], $fechaSII, $_SESSION["generales"]["cajero"], $usuCajero["idsede"]);
            }
            ...
            if ($reciboSII === false || $reciboSIIgob === false) {
                ...
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No fue posible generar el recibo de caja en el sistema (3) : ' . $_SESSION["generales"]["mensajeerror"]));
                $_SESSION["generales"]["codigousuario"] = $_SESSION["generales"]["codigousuariooriginal"];
                return $resultado;
            } else {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Recibo generado : ' . $reciboSII));
            }
        ...
        if ($result === false) {
            ...
            \logApi::general2($nameLog, $idSolicitudPago, $txtx);
            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('######################  REGISTRO DE PAGO TERMINADO CON ERROR #####################'));
            $_SESSION["generales"]["codigousuario"] = $_SESSION["generales"]["codigousuariooriginal"];
            return $resultado;
        }
        ...
        if ($grupoServicios == 'CerEle') {
            ...
            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('###################### PROCESO CER-ELE TERMINADO #####################'));
            $_SESSION["generales"]["codigousuario"] = $_SESSION["generales"]["codigousuariooriginal"];
            return $resultado;
        }
        ...
        if ($resultado1 === false) {
            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error regrabando en mreg_liquidacion los datos del recibo de caja generado : ' . $_SESSION["generales"]["mensajeerror"]));
        }
        ...
        if (trim($creaRuta) != '') {
            ...
            if (
                ...
                if (trim($estadosRutaPredefinidosInscripcionDocumentos) == '') {
                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No localizo pasos de la ruta predefinidos'));
            ...
            if (!defined('CONTROLA_CODIGOSBARRAS') || CONTROLA_CODIGOSBARRAS == '' || CONTROLA_CODIGOSBARRAS == 'SII' || CONTROLA_CODIGOSBARRAS == 'SIREP') {
                ...
                if ($buscar == 'error') {
                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No fue posible crear el codigo de barras de la transacción'));
                }
            }
            ...
            if (defined('CONTROLA_CODIGOSBARRAS') && CONTROLA_CODIGOSBARRAS == 'POWERFILE') {
                ...
                if ($cbSII === false || ltrim(trim($cbSII), "0") == '') {
                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error recuperando codigo de barras desde PowerFile'));
                } else {
                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Codigo de barras asignado por PowerFile : ' . $cbSII));
                }
            }
            ...
            if ($cbSII && (string) $cbSII != '' && (string) $cbSII != '0') {
                ...
                if ($res === false) {
                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No fue posible actualizar la informacion en el codigo de barras ' . $_SESSION["generales"]["mensajeerror"]));
                }
            ...
            if ($respuestaRuta["codigoError"] != '0000') {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error creando ruta : ' . $respuestaRuta["msgError"]));
            } else {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Creo ruta CodBarras : ' . $respuestaRuta["codbarras"]));
            }
            ...
            if ($resx === false) {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error en la asignacion del codigo de barras (' . $_SESSION["tramite"]["numeroradicacion"] . ') a la liquidacion (' . $_SESSION["tramite"]["idliquidacion"] . ')'));
            } else {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('asigno del codigo de barras (' . $_SESSION["tramite"]["numeroradicacion"] . ') a la liquidacion (' . $_SESSION["tramite"]["idliquidacion"] . ')'));
            }
        ...
        // 2017-12-16: JINT
        // Se adiciona control para ley 1780, código de policia alto impacto y codigo de policía multas
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('asigna el tipo de beneficiario si es matricula o renovacion'));
        $benart7 = '';
        $benley1780 = '';
        if (
                $_SESSION["tramite"]["multadoponal"] != 'S' &&
        ...
        // ************************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('localiza el ultimo ano renovado si es renovacion'));
        if ($esRenovacion == 'si') {
            $arrMatsAct = array();
            $arrMatsRens = array();
        ...
        // 2017-12-16: JINT
        // Se adiciona control para ley 1780, código de policia alto impacto y codigo de policía multas
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('localiza la fecha de renovacion'));
        if (
                $_SESSION["tramite"]["multadoponal"] != 'S' &&
                $_SESSION["tramite"]["multadoponal"] != 'L' &&
                $_SESSION["tramite"]["controlactividadaltoimpacto"] != 'S' && ($_SESSION["tramite"]["cumplorequisitosbenley1780"] != 'S' ||
        ...
        // ************************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('actualiza datos de las matriculas en caso de renovacion siempre y cuando no hayan formularios'));
        if (
                $_SESSION["tramite"]["multadoponal"] != 'S' &&
                $_SESSION["tramite"]["multadoponal"] != 'L' &&
                // $_SESSION["tramite"]["controlactividadaltoimpacto"] != 'S' && 
            ...
            if ($esRenovacion == 'si') {
                if ($tieneformularios == 'no' || $registroInmediato == 'no') {
                    foreach ($arrMatsAct as $m) {
                        ...
                        if ($res === false) {
                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error poniendo estado en 4 en mreg_est_inscritos para la matricula  ' . ltrim($m, "0")));
                        }
                    }
                }
                    ...
                    foreach ($arrMatsRens as $m => $a) {
                        if (trim($a) != '') {
                            if (substr(strtoupper($_SESSION["tramite"]["reliquidacion"]), 0, 1) != 'S') {
                                ...
                                if ($res === false) {
                                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error actualizando fecrenovacion en mreg_est_inscritos para la matricula  ' . ltrim($m, "0")));
                                }
                            }
                            // $res = \funcionesRegistrales::actualizarMregEstInscritosCampo($mysqli, ltrim($m, "0"), 'ultanoren', $a, 'varchar', $_SESSION["generales"]["codigobarras1"], $_SESSION["tramite"]["tipotramite"], $_SESSION["tramite"]["numerorecibo"]);
                            // if ($res === false) {
                            //     \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error actualizando ultanoren en mreg_est_inscritos para la matricula  ' . ltrim($m, "0")));
                            // }
                            ...
                            if ($res === false) {
                                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error actualizando anodatos en mreg_est_inscritos para la matricula  ' . ltrim($m, "0")));
                            }
                            ...
                            if ($res === false) {
                                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error actualizando fecdatos en mreg_est_inscritos para la matricula  ' . ltrim($m, "0")));
                            }
                            if ($arrMatsActivos[$m] != 0) {
                                ...
                                if ($res === false) {
                                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error actualizando acttot en mreg_est_inscritos para la matricula  ' . ltrim($m, "0")));
                                }
                                ...
                                if ($res === false) {
                                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error actualizando paspat en mreg_est_inscritos para la matricula  ' . ltrim($m, "0")));
                                }
                            ...
                            if ($arrMatsActVin[$m] != 0) {
                                ...
                                if ($res === false) {
                                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error actualizando actvin en mreg_est_inscritos para la matricula  ' . ltrim($m, "0")));
                                }
                            }
                            ...
                            if ($res === false) {
                                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error actualizando personal en mreg_est_inscritos para la matricula  ' . ltrim($m, "0")));
                            }
        ...
        // Se elimina el control de alto impacto.     
        // ************************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('actualiza informacion financiera en caso de renovacion siempre y cuando no hayan formularios'));
        if (
                $_SESSION["tramite"]["multadoponal"] != 'S' &&
                $_SESSION["tramite"]["multadoponal"] != 'L' &&
                // $_SESSION["tramite"]["controlactividadaltoimpacto"] != 'S' && 
                        ...
                        if (isset($d["idservicio"]) && $d["idservicio"] != '') {
                            if (substr($d["idservicio"], 0, 6) == '010202') {
                                if ($d["cc"] == '' || $d["cc"] == CODIGO_EMPRESA) {
                                    ...
                                    if ($resx === false) {
                                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error creando informacion financiera para ' . $d["expediente"] . ' - ' . $d["ano"] . ' - ' . date("Ymd") . ' - ' . $d["valorbase"]));
                                    }
                                }
                            }
                        }
        ...
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('actualiza datos cuando hay formularios en caso matriculas'));
        if (
                $_SESSION["tramite"]["multadoponal"] != 'S' &&
                $_SESSION["tramite"]["multadoponal"] != 'L' &&
                $_SESSION["tramite"]["controlactividadaltoimpacto"] != 'S' &&
            ...
            if ($asignaMatricula == 'si' && $registroInmediato == 'si' && $grupoServicios != 'RUE') {
                ...
                foreach ($arregloForms1 as $x) {
                    ...
                    // \logApi::general2($nameLog, $_SESSION["tramite"]["idliquidacion"], utf8_encode('Re-Actualizara formulario de la matricula ' . $x["datos"]["matricula"] . ': ' . $x["xml"]));
        ...
        // ************************************************************************************************************** //
        $formulariosactualizados = 0;
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('actualiza datos cuando hay formularios en caso de renovacion y/o matriculas'));
        if (
                $_SESSION["tramite"]["multadoponal"] != 'S' &&
                $_SESSION["tramite"]["multadoponal"] != 'L' &&
                // $_SESSION["tramite"]["controlactividadaltoimpacto"] != 'S' && 
            ...
            if ($esRenovacion == 'si' && $registroInmediato == 'si' && $grupoServicios != 'RUE') {
                ...
                foreach ($arregloForms1 as $x) {
                    ...
                    \logApi::general2($nameLog, $_SESSION["tramite"]["idliquidacion"], utf8_encode('Re-Actualizara formulario de la renovacion ' . $x["datos"]["matricula"] . ': ' . $x["xml"]));
                    if (!isset($_SESSION["tramite"]["tipomatricula"])) {
                        $_SESSION["tramite"]["tipomatricula"] = '';
                    }
        ...
        // Se adiciona control para ley 1780, código de policia alto impacto y codigo de policía multas
        // ************************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('actualiza datos cuando hay formularios en caso de mutacion'));
        if (
                $_SESSION["tramite"]["multadoponal"] != 'S' &&
                $_SESSION["tramite"]["multadoponal"] != 'L' &&
                $_SESSION["tramite"]["controlactividadaltoimpacto"] != 'S' && ($_SESSION["tramite"]["cumplorequisitosbenley1780"] != 'S' ||
        ...
        // 2017-12-16: JINT
        // Se adiciona control para ley 1780, código de policia alto impacto y codigo de policía multas
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('actualiza datos cuando no hay formularios en caso de matriculas'));
        if (
                $_SESSION["tramite"]["multadoponal"] != 'S' &&
                $_SESSION["tramite"]["multadoponal"] != 'L' &&
                $_SESSION["tramite"]["controlactividadaltoimpacto"] != 'S' && ($_SESSION["tramite"]["cumplorequisitosbenley1780"] != 'S' ||
        ...
        // Se adiciona control para ley 1780, código de policia alto impacto y codigo de policía multas
        // ************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('actualiza numero del recibo y codigo de barras al expediente  en caso de matriculas'));
        if (
                $_SESSION["tramite"]["multadoponal"] != 'S' &&
                $_SESSION["tramite"]["multadoponal"] != 'L' &&
                $_SESSION["tramite"]["controlactividadaltoimpacto"] != 'S' && ($_SESSION["tramite"]["cumplorequisitosbenley1780"] != 'S' ||
        ...
        // ************************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('actualiza estado de los datos en caso de matricula'));
        ...
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('actualiza datos de la forma de pago cuando sea pago electronico'));
        if (
                $_SESSION["tramite"]["idestado"] != '20' &&
                $_SESSION["tramite"]["idestado"] != '21' &&
                $_SESSION["tramite"]["idestado"] != '22' &&
        ...
        // ************************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Arma lista de inscripciones (matriculas)'));
        if ($asignaMatricula == 'si' && $registroInmediato == 'si') {
            // 2017-12-16: JINT
            // Se adiciona control para ley 1780, código de policia alto impacto y codigo de policía multas
            if (
        ...
        // ************************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Arma lista de inscripciones (mutaciones)'));
        ...
        // ************************************************************************************************************** //  
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Arma lista de inscripciones (reliquidacion de activos)'));
        ...
        //  ************************************************************************************************ //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Actualiza mreg_radicacionesdatos desde mreg_liquidaciondatos cuando sea un tramite de proponentes'));
        if ($grupoServicios == 'RegPro') {
            ...
            if ($xmlDatos != '0' && $xmlDatos != '') {
                ...
                if ($resx === false) {
                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error en traslado XML desde mreg_liquidaciondatos a mreg_radicionesdatos, codigo de barras ' . $_SESSION["generales"]["codigobarras1"] . ' : ' . $xmlDatos));
                }
            } else {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No encontro XML del tramite de proponentes para trasladar desde mreg_liquidaciondatos a mreg_radicacionesdatos, codigo de barras ' . $_SESSION["generales"]["codigobarras1"]));
            }
        }
        ...
        //  ************************************************************************************* //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Actualiza la cancelacion automatica cuando sea un tramite de cancelacion y su registro sea inmediato'));
            ...
            foreach ($_SESSION["tramite"]["transacciones"] as $tradat) {
                ...
                if ($tiptra["escancelacion"] == 'si' && $tiptra["registroinmediato"] == 'si') {
                    if ($_SESSION["generales"]["tipousuario"] != '00' && $_SESSION["generales"]["tipousuario"] != '06') {
                        ...
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Actualizado estado de la matricula a MC/IC/MF : ' . $xInscripciones[$xInsc]["matricula"]));

                        // 2016-08-26 : JINT : Actualiza fecha de cancelacion en mreg_est_inscritos
                        \funcionesRegistrales::actualizarMregEstInscritosCampo($mysqli, $xInscripciones[$xInsc]["matricula"], 'feccancelacion', date("Ymd"), 'varchar', $_SESSION["generales"]["codigobarras1"], $_SESSION["tramite"]["tipotramite"], $_SESSION["tramite"]["numerorecibo"]);
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Actualizado fecha de cancelacion en mreg_est_inscritos : ' . $xInscripciones[$xInsc]["matricula"]));

                        // 2016-11-30 : JINT : En caso de un afiliado, lo marca como ex-afiliado
                        //if (retornarRegistro('mreg_est_inscritos', "matricula='" . $xInscripciones[$xInsc]["matricula"] . "'", "ctrafiliacion") == '1') {
                        if (retornarRegistroMysqliApi($mysqli, "mreg_est_inscritos", "matricula='" . $xInscripciones[$xInsc]["matricula"] . "'", "ctrafiliacion") == '1') {
                            \funcionesRegistrales::actualizarMregEstInscritosCampo($mysqli, $xInscripciones[$xInsc]["matricula"], 'ctrafiliacion', 2, 'varchar', $_SESSION["generales"]["codigobarras1"], $_SESSION["tramite"]["tipotramite"], $_SESSION["tramite"]["numerorecibo"]);
                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Actualizado estado de ctrafiliacion a 2 en mreg_est_inscritos : ' . $xInscripciones[$xInsc]["matricula"]));
                        }
                    }
                }
            }
        ...
        // ******************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Genera inscripciones en libros'));
        if ($xInsc != 0) {
            $xInsc = 0;
            foreach ($xInscripciones as $x) {
                ...
                if (ltrim($numInsc, "0") == '' || $numInsc === false) {
                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error recuperando secuencia en mreg_secuencias para el libro : ' . $x["libro"]) . ' ' . $_SESSION["generales"]["mensajeerror"]);
                }
                ...
                if ($numInsc && $numInsc > 0) {
                    ...
                    //
                    if ($res === false) {
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error generando inscripciones en libros (mreg_est_inscripciones) : ' . $txt . "\r\n" . $_SESSION["generales"]["mensajeerror"]));
                    }
                    $xInsc++;
                    if ($respuestaLibro["codigoError"] != '0000') {
                        ...
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error generando inscripciones en libros : ' . $txt . "\r\n" . $respuestaLibro["msgError"]));
                    } else {
                        ...
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Genero inscripciones en libros : ' . $txt . "\r\n"));
                        ...
                        if ($resI === false) {
                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error insertando acto en mreg_estudio_actos_registro : ' . $_SESSION["generales"]["mensajeerror"] . "\r\n"));
                        }
                    }
                }
            }
        ...
        // ******************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Genera sellos soporte de las inscripciones en libros'));
                ...
                foreach ($xInscripciones as $x) {
                    ...
                    if (ltrim($x["numreg"], "0") != '') {
                        ...
                        if ($fx) {
                            ...
                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error generando sello para : ' . $x["tiporegistro"] . ' - ' . substr($x["libro"], 2, 2) . ' - ' . sprintf("%08s", $x["numreg"]) . "\r\n"));
                        }
                        $texto = '';
                        ...
                        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Genero Sello : Libro: ' . ltrim(substr($x["libro"], 2, 2), "0") . ' - Numreg: ' . ltrim($x["numreg"], "0") . ' - Dupli: ' . $x["dupli"] . ' - IdAnexo: ' . $id . ' - Path: ' . $pathsalida));
                        //
        ...
        // ******************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Actualiza el periodico de inscripciones en el codigo de barras'));
        if ($xInsc > 0) {
            $texto = '';
            foreach ($xInscripciones as $x) {
            ...
            //
            // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Actualiza inscripciones en codigos de barras  ' . $_SESSION["generales"]["codigobarras1"] . ' - Texto: ' . $texto . ' - Ope: ' . $x["ope"]));
        }
        ...
        // ************************************************************************************************* //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Actualiza tabla de alertas marcando como AP aplicada la alerta en cuestion'));
        ...
        // ************************************************************************************************* //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Actualiza prepagos'));
        $claveprepago = '';
        $saldoprepago = 0;
        ...
        // ************************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Actualiza prepagos movimientos cuando sea con cargo al mismo'));
        if ($_SESSION["tramite"]["pagoprepago"] != 0) {
            ...
            if ($resx === false) {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error Actualizando consumo de prepago en mreg_prepagos_uso, valor : ' . $_SESSION["tramite"]["pagoprepago"]));
            }
        ...
        // 2017-12-16: JINT
        // Se adiciona control para ley 1780, código de policia alto impacto y codigo de policía multas
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('En caso de mutacion de nombre crea el historico del nombre en SII  '));
        if (
                $_SESSION["tramite"]["multadoponal"] != 'S' &&
                $_SESSION["tramite"]["multadoponal"] != 'L' &&
                $_SESSION["tramite"]["controlactividadaltoimpacto"] != 'S' && ($_SESSION["tramite"]["cumplorequisitosbenley1780"] != 'S' ||
            ...
            if (($cambioNombre == 'si' && $_SESSION["tramite"]["tipotramite"] != 'inscripciondocumentos') || ($cambioNombre == 'si' && $_SESSION["tramite"]["tipotramite"] == 'inscripciondocumentos' && $registroInmediato == 'si')
                ...
                foreach ($xInscripciones as $x) {
                    ...
                    if ($resx === false) {
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error actualizando la tabla mreg_est_nombresanteriores'));
                    }
                }
            }
        }
        ...
        // ******************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('En caso de tramites RUES como receptora, Crea la tabla rue_radicacion en SII, Crea la tabla de certificados virtuales'));
        if ($tipoRegistro == 'RueRec' ||
                (substr($_SESSION["tramite"]["tipotramite"], 0, 4) == 'rues' &&
            ...
            if ($resx === false) {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error grabando mreg_rue_radicacion (receptora): ' . $_SESSION["generales"]["mensajeerror"]));
            }
            ...
            if ($resx === false) {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error grabando mreg_rue_radicacion_estados (receptora): ' . $_SESSION["generales"]["mensajeerror"]));
            }
            ...
            if ($resx === false) {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error grabando mreg_rue_radicacion_servicios (receptora) : ' . $_SESSION["generales"]["mensajeerror"]));
            }
            ...
            if (substr($_SESSION["tramite"]["tipotramite"], 0, 6) == 'rues01') {
                ...
                if (ltrim($respRues["camara_receptora"], "0") != '') {
                    ...
                    foreach ($arrTx as $tx) {
                        $sec++;
                        $nomcer = "RUES-" . date("Ymd") . '-' . date("His") . '-' . sprintf("%03s", $sec);
                        ...
                        if (!file_exists(PATH_ABSOLUTO_IMAGES . '/' . $_SESSION["generales"]["codigoempresa"] . '/' . $pathx)) {
                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error Almacenando el archivo : ' . PATH_ABSOLUTO_IMAGES . '/' . $_SESSION["generales"]["codigoempresa"] . '/' . $pathx));
                        }
                    ...
                    // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('El tramite genero ' . $sec . ' certificados para descargar'));
                }
            }
        }
        ...
        // Crea la tabla de certificados virtuales
        // ******************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('En caso de tramites RUES como responsable, Crea la tabla rue_radicacion en SII, Crea la tabla de certificados virtuales'));
        if (
                substr($_SESSION["tramite"]["tipotramite"], 0, 4) == 'rues' &&
                substr($_SESSION["tramite"]["tipotramite"], 6) == 'responsable'
        ) {
            ...
            if ($res1 == false) {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error creando mreg_rue_radicacion (responsable) : ' . $_SESSION["generales"]["mensajeerror"]));
            }
            ...
            if ($res1 == false) {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error creando mreg_rue_radicacion_estados (responsable) : ' . $_SESSION["generales"]["mensajeerror"]));
            }
            ...
            if ($res1 == false) {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error creando mreg_rue_radicacion_servicios (responsable) : ' . $_SESSION["generales"]["mensajeerror"]));
            }
        }
        ...
        // ******************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Genera Soportes de Caja en el Repositorio'));        
        \funcionesRegistrales::generarReciboCajaRepositorio($mysqli, $matricularecibo, $proponenterecibo, array(), $tiporeg, 'SII', 'S');
        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Genero recibo de caja en repositorio (Servicios)'));
        ...
        if (trim((string) $_SESSION["tramite"]["numerorecibogob"]) != '') {
            ...
            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Genero recibo de caja en repositorio (Gobernacion)'));
        }
        ...
        // ************************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Adiciona registros a la tabla mreg_estudio_actos_registro '));
                ...
                if ($arrTemX && !empty($arrTemX)) {
                    if ($arrTemX["idacto1"] != '') {
                        ...
                        if ($controlacto == 'si') {
                            ...
                            if ($xres === false) {
                                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error dicionando a mreg_estudio_actos_registro (1)'));
                            }
                        }
                    }
                    if ($arrTemX["idacto2"] != '') {
                        ...
                        if ($controlacto == 'si') {
                            ...
                            if ($xres === false) {
                                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error dicionando a mreg_estudio_actos_registro (2)'));
                            }
                        }
                    }
                    if ($arrTemX["idacto3"] != '') {
                        ...
                        if ($controlacto == 'si') {
                            ...
                            if ($xres === false) {
                                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error dicionando a mreg_estudio_actos_registro (3)'));
                            }
                        }
                    }
                    if ($arrTemX["idacto4"] != '') {
                        ...
                        if ($controlacto == 'si') {
                            ...
                            if ($xres === false) {
                                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error dicionando a mreg_estudio_actos_registro (4)'));
                            }
                        }
                    }
                    if ($arrTemX["idacto5"] != '') {
                        ...
                        if ($controlacto == 'si') {
                            ...
                            if ($xres === false) {
                                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error dicionando a mreg_estudio_actos_registro (5)'));
                            }
                        }
                    }
                }
        ...
        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Actos adicionados a mreg_estudio_actos_registro - ' . $iActos));

        // ************************************************************************************************************** //
        // 2016-10-19 : JINT
        // Si el pago es con cargo al cupo de afiliados, descuenta del cupo
        // ************************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Si el pago es con cargo al cupo de afiliados, descuenta del cupo'));
                ...
                if (!isset($_SESSION["tramite"]["tiporenovacion"]) || $_SESSION["tramite"]["tiporenovacion"] != 'prediligenciados') {
                    ...
                    if (trim($_SESSION["tramite"]["firmadoelectronicamente"]) != 'si' && trim($_SESSION["tramite"]["firmadomanuscrita"]) != 'si') {
                        if (retornarClaveValorMysqliApi($mysqli, '90.01.61') != 'SI') {
                            // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Entro a generar formularios NO FIRMADOS en el repositorio'));
                            \funcionesRegistrales::generarFormularioRenovacionRepositorio($mysqli, '', $nameLog);
                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Genero formularios NO FIRMADOS en el repositorio'));
                        } else {
                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No Genero formularios en el repositorio pues la clave valor 90.01.61 no esta activada'));
                        }
                    } else {
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No Genero formularios en el repositorio pues es un tramite firmado electronicamente/manuscrita'));
                    }
                }
            }
        }
        ...
        if ($arrTipoTramite["gruposervicios"] == 'RegMer' || $arrTipoTramite["gruposervicios"] == 'RegEsadl') {
            if ($arrTipoTramite["asignamatricula"] == 'si') {
                ...
                if (trim($_SESSION["tramite"]["firmadoelectronicamente"]) != 'si' && trim($_SESSION["tramite"]["firmadomanuscrita"]) != 'si') {
                    ...
                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No Genero formularios en el repositorio pues es un tramite firmado electronicamente'));
                }
            }
        }
        ...
        if ($arrTipoTramite["cambiodireccion"] == 'si') {
            if (trim($_SESSION["tramite"]["firmadoelectronicamente"]) != 'si' && trim($_SESSION["tramite"]["firmadomanuscrita"]) != 'si') {
                if (retornarClaveValorMysqliApi($mysqli, '90.01.61') != 'SI') {
                    ...
                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Genero formato de la mutacion en el repositorio'));
                } else {
                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No Genero formato de mutacion en el repositorio pues la clave valor 90.01.61 no esta activada'));
                }
            } else {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No Genero formularios en el repositorio pues es un tramite firmado electronicamente'));
            }
        ...
        // ******************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Traslada al repositorio los formatos y documentos anexos al tramite (liquidacion)'));
        ...
        if ($arrTem && !empty($arrTem)) {
            foreach ($arrTem as $t) {
                ...
                if (file_exists($namefx)) {
                    ...
                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Traslado soporte desde mreg_anexos_liquidaciones a mreg_radicacionesanexos : ' . $txt));
                }
            }
        }
        ...
        // ******************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Traslada al repositorio los formularios y solicitudes firmadas electronicamente'));
        ...
        // ******************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Incluye en el repositorio de imágenes el sobre digital si este existe'));
        ...
        if ($arrTem && !empty($arrTem)) {
            ...
            copy($namefx, PATH_ABSOLUTO_IMAGES . '/' . $_SESSION["generales"]["codigoempresa"] . '/' . $pathsalida);
            \funcionesRegistrales::grabarPathAnexoRadicacion($mysqli, $id, $pathsalida);
            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Traslado el sobre digital de la liquidación al repositorio : ' . $arrTem["path"]));
        }
        ...
        if (trim($_SESSION["tramite"]["identificacioncliente"]) != '222222222222') {
            // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Actualiza la tabla datos_empresas'));
            $arrCampos = array(
                'tipoidentificacion',
                'identificacion',
                'tipopersona',
        ...
        // ************************************************************************************************* //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Crea archivo en SII para Workflow'));
        ...
        if (GENERAR_WORKFLOW_SOPORTES == 'S' || GENERAR_WORKFLOW_SOPORTES == '') {
            ...
            //
            // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Entro a generar archivo del recibo para el workflow - operacion No. ' . $_SESSION["tramite"]["numerooperacion"] . ', recibo No. ' . $_SESSION["tramite"]["numerorecibo"]));
            ...
            if (isset($xInscripciones) && is_array($xInscripciones) && !empty($xInscripciones)) {
                foreach ($xInscripciones as $xi) {
                    // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Entro a generar archivo el sello para el workflow - Inscripcion No. ' . $xi["libro"] . sprintf("%08s", $xi["numreg"])));
        ...
        // ******************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('En caso de certificados especiales actualiza la tabla mreg_certificados_especiales'));
        if ($_SESSION["tramite"]["tipotramite"] == 'certificadosespeciales') {
            foreach ($_SESSION["tramite"]["liquidacion"] as $l) {
                if ($l["idservicio"] <= '01090151' || $l["idservicio"] >= '01090160') {
                    ...
                    if ($resx === false) {
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error almacenando certificados especiales'));
                    } else {
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Almaceno correctamente certificados especiales'));
                    }
                }
            }
        }
        ...
        if (
            ...
            if ($_SESSION["tramite"]["subtipotramite"] != 'matriculapjurcae' && $_SESSION["tramite"]["subtipotramite"] != 'matriculapnatcae') {
                if ($_SESSION["tramite"]["tipotramite"] == 'inscripciondocumentos') {
                    $gencert = 'no';
                } else {
                    if ($_SESSION["tramite"]["tipotramite"] == 'renovacionmatricula' || $_SESSION["tramite"]["tipotramite"] == 'renovacionesadl') {
            ...
            if ($gencert == 'no') {
                foreach ($_SESSION["tramite"]["liquidacion"] as $a) {
                    ...
                    if ($tipo != '') {
                        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Almaceno certificado para expedir posteriormente'));
                        $arrCampos = array(
                            'recibo',
                            'idliquidacion',
                            'fecha',
            ...
            if ($gencert == 'si') {
                ...
                foreach ($_SESSION["tramite"]["liquidacion"] as $a) {
                    ...
                    if ($tipo == '') {
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No localizo el tipo de certificado a generar asociado con el servicio ' . $a["idservicio"]));
                    }
                    $generarcert = 'si';
                    $controlarcert = 'no';
                    ...
                    if ($generarcert == 'si') {
                        if ($tipo != '') {
                            ...
                            //
                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Tipo de certificado a generar : ' . $tipo . ' Expediente ' . $a["expediente"]));
                            ...
                            if ($_SESSION["generales"]["cajero"] == 'USUPUBXXx' || $usuCajero["esbanco"] == 'SIx' || $_SESSION["tramite"]["enviara"] == 'Cx') {
                                ...
                                if ($resx === false) {
                                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error encolando certificados en pila_certificados para el recibo : ' . $_SESSION["tramite"]["numerorecibo"]));
                                }
                            }
                            ...
                            if ($certificadosencolados == 'no') {
                                ...
                                for ($cantCer = 1; $cantCer <= $a["cantidad"]; $cantCer++) {
                                    ...
                                    // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('genero numero aleatorio : ' . $aleatorio));
                                    if ($tipo == 'CerMat') {
                                    ...
                                    if ($tipo == 'CerEsadl') {
                                        if (defined('FECHA_INICIO_NUEVO_CERTIFICADO_CERESADL') && FECHA_INICIO_NUEVO_CERTIFICADO_CERESADL != '' && date("Ymd") >= FECHA_INICIO_NUEVO_CERTIFICADO_CERESADL && $exps["fechamatricula"] >= FECHA_INICIO_NUEVO_CERTIFICADO_CERESADL) {
                                            if ($exps["estadomatricula"] == 'IC') {
                                                ...
                                                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Genero certificado de ESADL formato antiguo en disco ' . $namex));
                                            } else {
                                                require_once($_SESSION["generales"]["pathabsoluto"] . '/api/genPdfsCertificadosFormato2019.php');
                                                ...
                                                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Genero certificado de ESADL formato 2019 en disco ' . $namex));
                                            }
                                        } else {
                                            if ($exps["estadomatricula"] == 'IC') {
                                                require_once($_SESSION["generales"]["pathabsoluto"] . '/api/genPdfsCertificadosNuevos.php');
                                    ...
                                    if ($tipo == 'CerLibRegMer') {
                                        ...
                                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Genero certificado de libros formato 2019 en disco ' . $namex));
                                    }
                                    ...
                                    if ($tipo == 'CerPro') {
                                        ...
                                        if (\funcionesGenerales::tamanoArchivo($_SESSION["generales"]["pathabsoluto"] . '/' . PATH_RELATIVO_IMAGES . '/' . $_SESSION["generales"]["codigoempresa"] . '/' . $namex) == 0) {
                                            ...
                                            $namex = generarCertificadosPdfProponentes($mysqli, $exps, $tipocertificadoadm, $a["valorservicio"] / $a["cantidad"], $_SESSION["tramite"]["numerooperacion"], $_SESSION["tramite"]["numerorecibo"], $aleatorio, $certificadoConsultaRues, $usuCajero["escajero"], $usuCajero["esbanco"], $firmar);
                                            if (\funcionesGenerales::tamanoArchivo($_SESSION["generales"]["pathabsoluto"] . '/' . PATH_RELATIVO_IMAGES . '/' . $_SESSION["generales"]["codigoempresa"] . '/' . $namex) != 0) {
                                                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Genero certificado de proponentes en disco en segundo intento ' . $namex));
                                            } else {
                                                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No fue posible generar el certificado de proponentes en disco en segundo intento, tamano cero ' . $namex));
                                                if ($_SESSION["generales"]["codigousuario"] != 'USUPUBXX') {
                                                    $namex = false;
                                                } else {
                                            ...
                                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Genero certificado de proponentes en disco en primer intento ' . $namex));
                                        }
                                    }
                                    ...
                                    if ($namex) {
                                        ...
                                        if ($_SESSION["generales"]["codigousuario"] != 'USUPUBXX') {
                                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error generando certificado en SII ' . $a["idservicio"]));
        ...
        if ($iCertificados != 0) {
            ...
            foreach ($arrCertificados as $cert) {
                ...
                if ($cfx && empty($cfx)) {
                    ...
                    if ($res === false) {
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error almacenando  mreg_certificados_virtuales : ' . $_SESSION["generales"]["mensajeerror"]));
                    }
                } else {
                    if (trim((string) $cfx["path"]) == '') {
                        ...
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error almacenando  mreg_certificados_virtuales, reescribira un id de certificado expedido previamente. Será necesario anular el recibo y regenarar.'));
                        $resultado["codigoError"] = '9999';
                ...
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Almaceno certificado generado  ' . $cert["idservicio"] . ' - ' . $cert["aleatorio"]));
            }
        }
        ...
        // ******************************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Envia emails de confirmacion tanto al usuario como al encargado de evaluar las transacciones electronicas o bancarias'));
        if (!isset($usuCajero)) {
            $usuCajero = array(
                'esbanco' => 'NO'
            );
        ...
        //
        if ($i > 0) {
            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Armo arreglo de soportes del recibo de caja (' . $i . ')'));
        }
                ...
                if ($xli["expediente"] != '') {
                    if (substr((string) $xli["expediente"], 0, 1) != 'S') {
                        ...
                        if ($expx1 && !empty($expx1)) {
                            if ($expx1["organizacion"] > '02' && $expx1["categopria"] == '1') {
                                $tienejuridicas = 'si';
                            }
                        }
                    }
        ...
        //
        if (!defined('ACTIVADO_DECRETO_399_RUP') || ACTIVADO_DECRETO_399_RUP == 'S') {
            if ($_SESSION["tramite"]["tipotramite"] === 'inscripcionproponente' || $_SESSION["tramite"]["tipotramite"] === 'renovacionproponente') {
                if (date("Y") === '2021') {
                    $txAnos = '2018 y 2019';
                }
                if (date("Y") === '2022') {
                ...
                $txt = 'Apreciado cliente, de acuerdo con lo establecido en el Decreto 399 de 2021, los proponentes que así lo crean pertinente y que ';
                $txt .= 'no tengan previamente reportada en el Registro de Proponentes la información financiera de los años ' . $txAnos . ' podrán ';
                $txt .= 'hacerlo, para el efecto le recomendamos comunicarse con nosotros para orientarlo.';
                $string .= $pres->armarLineaTextoInformativa($txt, 'center');
                $string .= $pres->cerrarPanel();
            }
        }
            ...
            if (
                ...
                if (TIPO_AMBIENTE === 'PRODUCCION' && $_SESSION["generales"]["emailusuariocontrol"] !== 'prueba@prueba.prueba') {
                    ...
                    if ($salidaemail === false) {
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error en envio email al pagador ' . $txtEmail . ', se reintentara'));
                        sleep(3);
                        $salidaemail = \funcionesGenerales::enviarEmail(SERVER_SMTP, SMTP_PORT, REQUIERE_SMTP_AUTENTICACION, SMTP_TIPO_ENCRIPCION, CUENTA_ADMIN_PORTAL, CLAVE_ADMIN_PORTAL, EMAIL_ADMIN_PORTAL, NOMBRE_ADMIN_PORTAL, $txtEmail, $asunto, $txtSalida, array());
                        if ($salidaemail === false) {
                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No fue posible enviar email al pagador ' . $txtEmail));
                        } else {
                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Enviado email al pagador ' . $txtEmail . ', segundo intento'));
                        ...
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Enviado email al pagador ' . $txtEmail));
                        \funcionesRegistrales::actualizarMregNotificacionesParaEnviarEmail($mysqli, '30', $_SESSION["tramite"]["numeroradicacion"], '', '', $_SESSION["tramite"]["numerorecibo"], '', '', '', '', $txtIdentificacion, '', '', $txtCliente, $txtEmail, $txtSalida, date("Ymd"), date("His"), date("Ymd"), date("His"), '3', '** OK **', '');
                    }
                } else {
                    ...
                    if ($salidaemail === false) {
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error en envio email al pagador ' . $emx . ' / ' . $txtEmail . ', se reintentara'));
                        sleep(3);
                        $salidaemail = \funcionesGenerales::enviarEmail(SERVER_SMTP, SMTP_PORT, REQUIERE_SMTP_AUTENTICACION, SMTP_TIPO_ENCRIPCION, CUENTA_ADMIN_PORTAL, CLAVE_ADMIN_PORTAL, EMAIL_ADMIN_PORTAL, NOMBRE_ADMIN_PORTAL, $emx, $asunto, $txtSalida, array());
                        if ($salidaemail === false) {
                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No fue posible enviar email al pagador ' . $emx . ' (' . $txtEmail . ')'));
                        } else {
                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Enviado email al pagador ' . $emx . ' (' . $txtEmail . '), segundo intento'));
                            \funcionesRegistrales::actualizarMregNotificacionesParaEnviarEmail($mysqli, '30', $_SESSION["tramite"]["numeroradicacion"], '', '', $_SESSION["tramite"]["numerorecibo"], '', '', '', '', $txtIdentificacion, '', '', $txtCliente, $txtEmail, $txtSalida, date("Ymd"), date("His"), date("Ymd"), date("His"), '3', '** OK **', '');
                        }
                    } else {
                        \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Enviado email al pagador ' . $emx . ' (' . $txtEmail . ')'));
                        \funcionesRegistrales::actualizarMregNotificacionesParaEnviarEmail($mysqli, '30', $_SESSION["tramite"]["numeroradicacion"], '', '', $_SESSION["tramite"]["numerorecibo"], '', '', '', '', $txtIdentificacion, '', '', $txtCliente, $txtEmail, $txtSalida, date("Ymd"), date("His"), date("Ymd"), date("His"), '3', '** OK **', '');
                    }
                }
            }
        ...
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Actualiza mreg_liquidacionflujo a 6'));
        // }
        // ****************************************************************************** //
        ...
        // ****************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('En caso de renovaciones nacionales, marca en mreg_establecimientos_nacionales aquellos expedientes que fueron radicados con el trámite, para el postrior consumo de MR02N'));
        if ($_SESSION["tramite"]["tipotramite"] === 'renovacionmatricula' || $_SESSION["tramite"]["tipotramite"] === 'renovacionesadl') {
        ...
        // *********************************************************************************************** //
        // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Notificacion de radicaciones'));
        $notificar = 'no';
        ...
        if ($ttx && !empty($ttx)) {
            if ($ttx["crearuta"] == '') {
                ...
                // ********************************************************************************************* //
                // 2018 07 26 - JINT: Valida si el servicio asociado al trámite es un embargo o medida cautelar
                // de ser así no lo notifica
                // 
                // 2018-09-20 - JINT: Ajustes de las rutas que deben o no ser notificables (SIPREF)
                // ********************************************************************************************* //
                if ($notificar === 'si') {
                    $ls = 0;
                    $lsno = 0;
        ...
        if ($_SESSION["tramite"]["tipotramite"] == 'depestfinregmer' || $_SESSION["tramite"]["tipotramite"] == 'depestfinesadl') {
            // \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('asocia datos del recibo a la tabla mreg_publicacion_balances'));
            $arrCampos = array(
                'codigobarras',
                'recibo',
                'fecharadicacion',
        ...
        if (defined('CFE_FECHA_INICIAL') && CFE_FECHA_INICIAL != '' && CFE_FECHA_INICIAL <= date("Ymd")) {
            ...
            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('ejecuto envio de factura electronica a CFE - Codigo : ' . $resCfe["codigoError"] . ' - Mensaje : ' . $resCfe["mensajeError"]));
            if ($totalgobernacion != 0) {
                $resCfe1 = \funcionesCFE::seleccionRecibosCFE($mysqli, '00000000', '000000', '000000', $result["numerorecibogob"]);
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('ejecuto envio de factura electronica a CFE - Codigo : ' . $resCfe1["codigoError"] . ' - Mensaje : ' . $resCfe1["mensajeError"]));
            }
        }
        ...
        if (
            ...
            if ($_SESSION["tramite"]["tipotramite"] != 'renovacionmatricula' && $_SESSION["tramite"]["tipotramite"] != 'renovacionesadl') {
                ...
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No actualiza el RUES'));
            }
        }
        if ($actrues == 'si' && ($grupoServicios == 'RegMer' || $grupoServicios == 'RegEsadl')) {
            ...
            if ($_SESSION["tramite"]["tipotramite"] != 'inscripciondocumentos' || ($_SESSION["tramite"]["tipotramite"] == 'inscripciondocumentos' && $registroInmediato == 'si')) {
                ...
                if (ACTUALIZAR_RUES_AL_PAGAR == 'S') {
                                ...
                                if (!defined('ACTUALIZAR_RUES_BACKGROUND') || ACTUALIZAR_RUES_BACKGROUND == '' || ACTUALIZAR_RUES_BACKGROUND == 'SI') {
                                    ...
                                    if ($m != '' && substr($m, 0, 5) != 'NUEVA') {
                                        ...
                                        if ($r["codigoError"] == '0000') {
                                            ...
                                            unset($_SESSION["expedienteactual"]);
                                            regrabarRegistrosMysqliApi($mysqli, 'mreg_est_inscritos', $arrCampos, $arrValores, "matricula='" . $m . "'");
                                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('RUES actualizado satisfactoriamente (' . $m . ')'));
                                        } else {
                                            \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error actualizando el RUES (' . $m . ') (' . $r["msgError"] . ')'));
                                        }
                    ...
                    \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No esta configurada la actualizacion del RUES al pagar en el commonXX'));
                }
            } else {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('No actualiza el RUES'));
            }
        ...
        if (ltrim((string) $cbSII, "0") != '') {
            ...
            $resx = regrabarRegistrosMysqliApi($mysqli, 'mreg_liquidacion', $arrCampos, $arrValues, $condicion);
            if ($resx === false) {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('Error final en la asignacion del codigo de barras (' . $_SESSION["tramite"]["numeroradicacion"] . ') a la liquidacion (' . $_SESSION["tramite"]["idliquidacion"] . ')'));
            } else {
                \logApi::general2($nameLog, $idSolicitudPago, utf8_encode('asignacion final del codigo de barras (' . $_SESSION["tramite"]["numeroradicacion"] . ') a la liquidacion (' . $_SESSION["tramite"]["idliquidacion"] . ')'));
            }
        }
