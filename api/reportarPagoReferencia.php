...
trait reportarPagoReferencia {
    ...
    public function reportarPagoReferencia (API $api) {
        ...
        require_once ('../../../api/gestionRecibos.php');
        ...
        // **************************************************************************** //
        $_SESSION["tramite"] = \funcionesSii2::retornarMregLiquidacionSii($mysqli, substr($_SESSION["entrada"]["idliquidacion"],2));
        ...
        //
        $tx = retornarRegistroMysqli2($mysqli, 'mreg_formaspago', "id='" . $_SESSION["entrada"]["formapago"] . "'");
        ...
        if ($_SESSION["entrada"]["formapago"] == '91') { // Con cargo a prepago
            ...
            $preps = retornarRegistroMysqli2($mysqli, 'mreg_prepagos', "identificacion='" . $_SESSION["entrada"]["identificacionprepagoafiliado"] . "'");
        ...
        if ($_SESSION["entrada"]["formapago"] == '92') {
            $matx = '';
            $cupo = 0;
            $exps = retornarRegistrosMysqli2($mysqli, 'mreg_est_inscritos', "numid='" . $_SESSION["entrada"]["identificacionprepagoafiliado"] . "' or nit='" . $_SESSION["entrada"]["identificacionprepagoafiliado"] . "'", "matricula");
            if ($exps && !empty($exps)) {
                foreach ($exps as $exp) {
            ...
            //
            $formacalculocupoafiliados = retornarClaveValorSii2($mysqli, '90.01.60');
            if ($formacalculocupoafiliados == 'CANTI_CERTIFICADOS') {
                $resx1 = \funcionesSii2::consultarSaldoAfiliadoCantidad($mysqli, $matx);
                if ($resx1 && !empty($resx1)) {
                    $cupo = $resx1["cupo"];
                }
            } else {
                $resx1 = \funcionesSii2::consultarSaldoAfiliado($mysqli, $matx);
                if ($resx1 && !empty($resx1)) {
                    $cupo = $resx1["cupo"];
                }
            }
            ...
            $afil = retornarRegistroMysqli($mysqli, 'mreg_claves_afiliados', "matricula='" . $matx . "'");
            ...
            $clavelimpia = \funcionesSii2::encrypt_decrypt('decrypt', 'c0nf3c4m4r4s', 'c0nf3c4m4r4s', trim($_SESSION["entrada1"]["claveprepagoafiliado"]));
        ...
        //
        $resultado = \funcionesSii2::grabarLiquidacionMregSii($mysqli);
        if (!$resultado) {
            $_SESSION["jsonsalida"]["codigoerror"] = '9999';
        ...
        if ($res["codigoError"] != '0000') {
            ...
            $certs = retornarRegistrosMysqli2($mysqli, 'mreg_certificados_virtuales', "recibo='" . $_SESSION["jsonsalida"]["numerorecibo"] . "'", "id");
