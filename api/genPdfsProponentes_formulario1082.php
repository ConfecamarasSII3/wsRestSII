...
/** * 
 * Función que realiza el armado de los formularios del registro de proponentes 
 ...
 * @return string
 */
function armarPdfFormularioProponentes1082Api($dbx, $datosForm = array(), $tipotramite = '', $numliq = '', $numrec = '', $nombrerepleg = '', $identificacionrepleg = '', $idtipoidentificacionrepleg = '', $tipoimpresion = 'final', $txtFirmaElectronica = '') {
    ...
    if ($tipoimpresion != 'vacio') {
        ...
        if (trim($aV ["ubicacion"]) == 'S') {
             ...
             * 
             */
            switch (substr($datosForm["ctrmen"], 0, 1)) {
                case "S" :
            ...
            /**
             * 
             * DATOS DE UBICACIÓN NO MODIFICADOS
             * 
             */
        ...
        if ($tipotramite != 'cancelacionproponente') {
            ...
            if (trim($aV ["inffin1510"]) == 'S') {
                /**
                 * 
                 * INFORMACION FINANCIERA CON CAMBIOS
                 *  
                 */
                $decimalesVisibles = 2;
                ...
                /**
                 * 
                 * DATOS DE INFORMACION FINANCIERA NO MODIFICADA
                 * 
                 */
        ...
        /*
         * 
         * $datosForm["gruponiif"] = del 0 al 7, donde 0 es no reportado
         *
         */
    ...
    if ($tipoimpresion != 'vacio' and $tipotramite != 'cancelacionproponente' && $tipotramite != 'cambiodomicilioproponente') {
        ...
        //$formulario->armarCampo('p6.sec2_inf1', $datosForm["gruponiiftextual"]);
        //$formulario->armarCampo('p6.sec2_inf2', $datosForm["marcotecnicoanterior"]);
        ...
        if (trim($aV["inffin1510"]) == 'S') {
            ...
            if ($datosForm["inffin1510_pascte"] == 0) {
                ...
                // $indiceLiquidez = truncarValorNuevo($datosForm["inffin1510_actcte"] / $datosForm["inffin1510_pascte"]);
                // $indiceLiquidez = truncarValorNuevoFormulario($datosForm["inffin1510_actcte"] / $datosForm["inffin1510_pascte"]);
...
//                $indiceEndeudamiento = truncarValorNuevoFormulario($datosForm["inffin1510_pastot"] / $datosForm["inffin1510_acttot"]);
            ...
            if ($datosForm["inffin1510_gasint"] != 0) {
                // $razonCobertura = truncarValorNuevo($datosForm["inffin1510_utiope"] / $datosForm["inffin1510_gasint"]);
    ...
    if (($tipoimpresion != 'vacio') and ( $tipotramite != 'cancelacionproponente')) {
        /**
         * 
         * CONTROLA QUE EL ARREGLO DE LAS SITUACIONES DE CONTROL NO SEA VACIO, DE LO CONTRARIO NO IMPRIME
         * 
         */
        if (count($datosForm["sitcontrol"]) != 0) {
            ...
            /**
             * 
             * Calcular el número de páginas a adicionar dinámicamente
             * 
             */
    ...
    if ($tipoimpresion != 'vacio' and $tipotramite != 'cancelacionproponente' && $tipotramite != 'cambiodomicilioproponente') {
        ...
        if (trim($aV ["clasi1510"]) == 'S') {
            ...
            for ($hoja_clasif = 1; $hoja_clasif <= $paginas_adicionales_clasif; $hoja_clasif ++) {
                ...
                /**
                 * 
                 * RECORRE ARREGLO DE CLASIFICACIONES
                 * 
                 */
                foreach ($datosForm["clasi1510"] as $clave => $valor) {
                    ...
                    /**
                     * 
                     * LEYENDA CONTINUA EN CLASIFICACIONES
                     * 
                     */
                    /*
                      if ($clave == $token_final_clasif) {
                      $formulario->armarCampo ( 'p8.continua', 'CONTINUA...', 1 );
                      }
                     */
    ...
    if ($tipoimpresion != 'vacio' and $tipotramite != 'cancelacionproponente' && $tipotramite != 'cambiodomicilioproponente') {
        ...
        /**
         * 
         * IMPRIME SEGUN CANTIDAD DE REPRESENTANTES LEGALES
         * 
         */
    ...
    if ($tipoimpresion != 'vacio' and $tipotramite != 'cancelacionproponente' && $tipotramite != 'cambiodomicilioproponente') {
        ...
        /**
         * 
         * IMPRIME SEGUN CANTIDAD DE  CONTRATOS NO ELIMINADOS
         * 
         */
        foreach ($datosForm["exp1510"] as $key => $valor) {
            ...
            if (in_array($valor ['secuencia'], $secuencia_imprimir)) {
                ...
                if (trim($aV [$sec]) == 'S') {
                  ...
                  // IMPRIME ÚNICA HOJA SI EL CONTRATO NO HA CAMBIADO
                  $formulario->agregarPagina(10, $tipoFormulario);
                  ...
                  $formulario->armarCampo('p10.nit', '--');
                  $formulario->armarCampo('p10.dv', '--');

                  $formulario->armarCampo('p10.num_cons_rep', $datosForm["exp1510"] [$key] ["secuencia"]);

                  switch ($datosForm["exp1510"] [$key] ["celebradopor"]) {
                  case "1" :
                  $formulario->armarCampo('p10.exp_prop', '--');
                  break;
                  case "2" :
                  $formulario->armarCampo('p10.exp_acc', '--');
                  break;
                  case "3" :
                  $formulario->armarCampo('p10.exp_cons', '--');
                  break;
                  }

                  $formulario->armarCampo('p10.nom_contratista', '--');
                  $formulario->armarCampo('p10.nom_contratante', '--');
                  $formulario->armarCampo('p10.val_cont', '--');
                  $formulario->armarCampo('p10.porc_part', '--');

                  for ($x = 1; $x <= 15; $x ++) {
                  $formulario->armarCampo('p10.cod_seg', '--', $x);
                  $formulario->armarCampo('p10.cod_fam', '--', $x);
                  $formulario->armarCampo('p10.cod_cla', '--', $x);
                  }
    ...
    if ($tipoimpresion != 'vacio') {
        if ($tipoimpresion == 'final') {
            ...
            if (BLOQUEAR_FORMULARIO_PROPONENTES_AL_IMPRIMIR == 'S') {
                if (retornarMregLiquidacionEstado($numliq) < '04') {
