 ...
 */
function armarPdfPrincipalNuevo1082Api($dbx = null, $numrec = '', $numliq = 0, $tipoimpresion = '', $prediligenciado = 'no', $txtFirmaElectronica = '', $txtFirmaManuscrita = '', $ideFirmaManuscrita = '', $nomFirmaManuscrita = '', $cambidom = 'no', $anoimprimir = '') {
    require_once ($_SESSION["generales"]["pathabsoluto"] . '/api/pdfFormularioRues-2017.php');
 ...
 * @return string
 */
function armarPdfPrincipalNuevo1082AnexoApi($dbx = null, $numrec = '', $numliq = 0, $mat = '', $tipo = '') {
    require_once ($_SESSION["generales"]["pathabsoluto"] . '/components/fpdf186/fpdf_protection.php');
    require_once ($_SESSION["generales"]["pathabsoluto"] . '/api/log.php');

    //
    if ($tipo == 'prediligenciados') {
        $datx = retornarRegistroMysqliApi($dbx, 'mreg_est_inscritos', "matricula='" . $mat . "'");
        $_SESSION["formulario"]["datos"] = $datx;
        $_SESSION["formulario"]["tipotramite"] = 'renovacionmatricula';
        $_SESSION["formulario"]["datos"]["nombre"] = $datx["razonsocial"];
    }
    $datosForm = $_SESSION ["formulario"]["datos"];
    ...
    if (!class_exists('PDFAnexo')) {

        class PDFAnexo extends FPDF {
    ...
    $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode('INFORMACIÓN ADICIONAL FORMULARIO DEL REGISTRO UNICO EMPRESARIAL Y SOCIAL RUES'), 0, 'C');
    $pdf->Ln();
    $pdf->Ln();
    ...
    $xmat = '   ';
    $xren = '   ';
    if ($datosForm["matricula"] != '' && substr($datosForm["matricula"], 0, 5) != 'NUEVA') {
        $matx = $datosForm["matricula"];
    }
    if ($_SESSION["formulario"]["tipotramite"] == 'renovacionmatricula') {
        $xren = ' X ';
    } else {
    ...
    $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode('Fecha : ' . date("Y-m-d") . '      Matrícula [' . $xmat . '] Renovación [' . $xren . '] Matrícula No. ' . $matx), 0, 'L');
    $pdf->Ln();

    //
    $pdf->SetFont('Courier', 'B', 8);
    $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode('Nombre o Razón Social ' . $datosForm["nombre"]), 0, 'L');
    $pdf->Ln();
    $pdf->Ln();

    //
    $pdf->SetFont('Courier', '', 8);
    $txt = 'Diligencie la siguiente información exigida en la instrucción de la Superintendencia de Industria y Comercio Rad. 19-287533-0-0, en cada uno de los items ';
    $txt .= 'señalados según corresponda:';
    $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode($txt), 0, 'J');
    $pdf->Ln();

    //
    $pdf->SetFont('Courier', 'B', 8);
    $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode('1. DILIGENCIE ESTE ESPACIO SI ES PERSONA NATURAL'), 0, 'L');
    $pdf->Ln();
    ...
    $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode('1.1. Género                           Masculino [ ' . $sexM . ' ]   Femenino [ ' . $sexF . ' ]'), 0, 'L');
    $pdf->Ln();

    //
    $pdf->SetFont('Courier', '', 8);
    if ($datosForm["organizacion"] == '01' && $tipo != 'prediligenciados') {
        $pdf->MultiCell(150, 4, \funcionesGenerales::utf8_decode('1.2. Indique el número total de mujeres empleadas en la empresa : ' . $datosForm["cantidadmujeres"]), 0, 'L');
    } else {
        $pdf->MultiCell(150, 4, \funcionesGenerales::utf8_decode('1.2. Indique el número total de mujeres empleadas en la empresa : ________'), 0, 'L');
    }
    $pdf->Ln();
    $pdf->Ln();
    ...
    //
    $pdf->SetFont('Courier', 'B', 8);
    $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode('2. DILIGENCIE ESTE ESPACIO SI ES PERSONA JURÍDICA'), 0, 'L');
    $pdf->Ln();

    $pdf->SetFont('Courier', '', 8);
    if ($datosForm["organizacion"] > '02' && $datosForm["categoria"] == '1' && $datosForm["organizacion"] != '12' && $datosForm["organizacion"] != '14' && $tipo != 'prediligenciados') {
        $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode('2.1. Indique el porcentaje de participación de las mujeres en el capital social : ' . $datosForm["participacionmujeres"] . '%'), 0, 'L');
    } else {
        $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode('2.1. Indique el porcentaje de participación de las mujeres en el capital social : _______ %'), 0, 'L');
    }
    $pdf->Ln();

    $pdf->SetFont('Courier', '', 8);
    $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode('(*) Este campo no aplica para ESADL.'), 0, 'L');
    $pdf->Ln();

    $pdf->SetFont('Courier', '', 8);
    if ($datosForm["organizacion"] > '02' && $datosForm["categoria"] == '1' && $tipo != 'prediligenciados') {
        $pdf->MultiCell(150, 4, \funcionesGenerales::utf8_decode('2.2. Indique el número de mujeres que ocupan cargos directivos : ' . $datosForm["cantidadmujerescargosdirectivos"]), 0, 'L');
    } else {
        $pdf->MultiCell(150, 4, \funcionesGenerales::utf8_decode('2.2. Indique el número de mujeres que ocupan cargos directivos : ______'), 0, 'L');
    }
    $pdf->Ln();

    $pdf->SetFont('Courier', '', 8);
    if ($datosForm["organizacion"] > '02' && $datosForm["categoria"] == '1' && $tipo != 'prediligenciados') {
        $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode('2.3. Indique el número total de mujeres empleadas en la empresa : ' . $datosForm["cantidadmujeres"]), 0, 'L');
    } else {
        $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode('2.3. Indique el número total de mujeres empleadas en la empresa : _______'), 0, 'L');
    }
    $pdf->Ln();
    $pdf->Ln();
    ...
    //
    $pdf->SetFont('Courier', 'B', 8);
    $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode('3. DILIGENCIE ESTE ESPACIO SI INDICA DOS O MÁS CÓDIGOS CIIU'), 0, 'L');
    $pdf->Ln();

    //
    $txt = '3.1. En el evento en que en el formulario RUES se hayan relacionado en su actividad económica dos o mas códigos CIIU, indique a continuación el CIIU ';
    $txt .= 'por el cual recibió mayores ingresos por actividad ordinaria: ';
    if ($tipo != 'prediligenciados') {
        $txt .= $datosForm["ciiutamanoempresarial"];
    }
    $pdf->SetFont('Courier', '', 8);
    $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode($txt), 0, 'J');
    $pdf->Ln();
    ...
    //
    $txt = 'La información antes suministrada hace parte del trámite solicitado y se entiende firmado con la suscripción del formulario RUES.';
    $pdf->SetFont('Courier', '', 8);
    $pdf->MultiCell(180, 4, \funcionesGenerales::utf8_decode($txt), 0, 'J');
    $pdf->Ln();
    ...
    //
    if ($tipo == 'prediligenciados') {
        $name = $_SESSION["generales"]["pathabsoluto"] . '/tmp/' . $_SESSION["generales"]["codigoempresa"] . '-AnexoFormularioPrediligenciados-' . $mat . '.pdf';
        $name1 = $_SESSION["generales"]["codigoempresa"] . '-AnexoFormularioPrediligenciados-' . $mat . '.pdf';
        $pdf->Output($name, "F");
    } else {
        $name = $_SESSION["generales"]["pathabsoluto"] . '/tmp/' . $_SESSION["generales"]["codigoempresa"] . '-AnexoFormulario-' . $numliq . '.pdf';
        $name1 = $_SESSION["generales"]["codigoempresa"] . '-AnexoFormulario-' . $numliq . '.pdf';
        $pdf->Output($name, "F");
    }

    \logApi::general2('GeneracionAnexoFormulario_' . date("Ymd"), $numliq, 'Genero anexo al formulario : ' . $name1);
    return $name1;
}
