        ...
        if (!empty($resQueryAnexos)) {
            foreach ($resQueryAnexos as $imagenInfo) {
                if ($imagenInfo['sistemaorigen'] == 'DOCUWARE') {
                    $imagenx = TIPO_HTTP . HTTP_HOST . '/tmp/' . str_replace("../../../tmp/", "", \funcionesGenerales::recuperarImagenRepositorio($imagenInfo['url'], 'DOCUWARE', $imagenInfo['numeropaginas'], $imagenInfo['idorigenexterno']));
                } else {
                    if (TIPO_REPOSITORIO_NUEVAS_IMAGENES == 'EFS_S3') {
                        $imagenx = obtenerUrlRepositorioS3Api($imagenInfo['url']);
                        // $imagenx = TIPO_HTTP . HTTP_HOST . '/tmp/' . str_replace("../../../tmp/", "", \funcionesGenerales::recuperarImagenRepositorio($imagenInfo['url']));
                    } else {
