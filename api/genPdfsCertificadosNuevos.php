...
function generarCertificadosPdfEspeciales($mysqli, $data, $i, $cuerpo, $tipo = 'Consulta', $firmar = 'si') {
    ...
    if (!class_exists('PDFRequeEspe1')) {

        class PDFRequeEspe1 extends TCPDF {
            ...
            public function Header() {
                ...
                //
                // if ($this->tituloTipo == 'Consulta' || $this->tituloTipo == 'ConsultaD' || $this->tituloTipo == 'Api') {
    ...
    // Instanciamiento
    // ****************************************************************************** //
    $pdf = new PDFRequeEspe1(PDF_PAGE_ORIENTATION, PDF_UNIT, 'LETTER', true, 'UTF-8', false, true);
    ...
    // *************************************************************************** //    
    armarTextoElecciones($pdf, $mysqli);
    ...
    if ($data["codigoverificacion"] == '') {
        if ($pdf->tituloTipo == 'Consulta') {
            ...
            if ($pdf->tituloTipo == 'ConsultaD') {
                $aleatorio = \funcionesGenerales::generarAleatorioAlfanumerico10($mysqli);
                $name = $_SESSION["generales"]["codigoempresa"] . '-ConsultaCertificadoEspecial-' . $aleatorio . '.pdf';
                ...
                if ($pdf->tituloTipo == 'Api-Consulta') {
                    $aleatorio = \funcionesGenerales::generarAleatorioAlfanumerico10($mysqli);
                    $name = 'tmp/' . $_SESSION["generales"]["codigoempresa"] . '-ConsultaCertificadoEspecial-' . $aleatorio . '.pdf';
                    $name1 = $_SESSION["generales"]["pathabsoluto"] . '/' . $name;
                    $pdf->Output($name1, "D");
                    return $name;
                } else {
                    $aleatorio = \funcionesGenerales::generarAleatorioAlfanumerico10($mysqli);
...
function generarCertificadosPdfMatricula($mysqli, $data, $tipo, $valorCertificado = 0, $operacion = '', $recibo = '', $aleatorio = '', $certificadoConsultaRues = 'no', $escajero = 'SI', $esbanco = 'NO', $firmar = '') {
    ...
    if (!class_exists('PDFRequeMat1')) {

        class PDFRequeMat1 extends TCPDF {
    ...
    // Instanciamiento
    // ****************************************************************************** //
    $pdf = new PDFRequeMat1(PDF_PAGE_ORIENTATION, PDF_UNIT, 'LETTER', true, 'UTF-8', false, true);
    if (defined('TAMANO_LETRA_CERTIFICADOS_SII') && trim(TAMANO_LETRA_CERTIFICADOS_SII) != '') {
        $pdf->tamanoLetra = TAMANO_LETRA_CERTIFICADOS_SII;
    }
    ...
    //
    // armarCertificaEnDepuracion1727($pdf,$data,$mysqli);
    ...
    if ($pdf->tituloTipo == 'ConsultaD') {
        $name = $_SESSION["generales"]["codigoempresa"] . '-ConsultaCertificado-' . $aleatorio . '.pdf';
        $name1 = $_SESSION["generales"]["pathabsoluto"] . '/tmp/' . $name;
        $pdf->Output($name1, "F");
        ob_end_clean();
        return $name;
    ...
    if ($pdf->tituloTipo == 'Api-Consulta') {
        $name = $_SESSION["generales"]["pathabsoluto"] . '/tmp/' . $_SESSION["generales"]["codigoempresa"] . '-ConsultaCertificadoApi-' . $aleatorio . '.pdf';
        $name1 = 'tmp/' . $_SESSION["generales"]["codigoempresa"] . '-ConsultaCertificadoApi-' . $aleatorio . '.pdf';
        $pdf->Output($name, "F");
        ob_end_clean();
        return $name1;
    }
    ...
    $anox = date("Y");
    $mesx = sprintf("%02s", date("m"));
    if (!is_dir(PATH_ABSOLUTO_SITIO . '/' . PATH_RELATIVO_IMAGES . '/' . $_SESSION["generales"]["codigoempresa"] . '/mreg/')) {
        mkdir(PATH_ABSOLUTO_SITIO . '/' . PATH_RELATIVO_IMAGES . '/' . $_SESSION["generales"]["codigoempresa"] . '/mreg/', 0777);
        \funcionesGenerales::crearIndex(PATH_ABSOLUTO_SITIO . '/' . PATH_RELATIVO_IMAGES . '/' . $_SESSION["generales"]["codigoempresa"] . '/mreg/');
    }
...
function generarCertificadosPdfExistencia($mysqli, $data, $tipo, $valorCertificado = 0, $operacion = '', $recibo = '', $aleatorio = '', $certificadoConsultaRues = 'no', $escajero = 'SI', $esbanco = 'NO', $firmar = '') {
    ...
    if (!class_exists('PDFRequeExi1')) {

        class PDFRequeExi1 extends TCPDF {
    ...
    // Instanciamiento
    // ****************************************************************************** //
    $pdf = new PDFRequeExi1(PDF_PAGE_ORIENTATION, PDF_UNIT, 'LETTER', true, 'UTF-8', false, true);
    if (defined('TAMANO_LETRA_CERTIFICADOS_SII') && trim(TAMANO_LETRA_CERTIFICADOS_SII) != '') {
        $pdf->tamanoLetra = TAMANO_LETRA_CERTIFICADOS_SII;
    }
    $pdf->tituloEstadoMatricula = $data["estadomatricula"];
    ...
    //
    if ($pdf->tituloTipo == 'ConsultaD') {
        $name = $_SESSION["generales"]["codigoempresa"] . '-ConsultaCertificadoConsulta-' . $aleatorio . '.pdf';
        $name1 = $_SESSION["generales"]["pathabsoluto"] . '/tmp/' . $name;
        $pdf->Output($name1, "F");
        ob_end_clean();
        unset($pdf);
...
function generarCertificadosPdfEsadl($mysqli, $data, $tipo, $valorCertificado = 0, $operacion = '', $recibo = '', $aleatorio = '', $certificadoConsultaRues = 'no', $escajero = 'SI', $esbanco = 'NO', $firmar = '') {
    ...
    if (!class_exists('PDFRequeEsadl1')) {

        class PDFRequeEsadl1 extends TCPDF {
            ...
            public function Header() {
                ...
                $this->SetFontSize(8);
                $this->SetTextColor(139, 0, 0);
                $this->MultiCell(195, 4, $this->tituloNombreCamara . chr(13) . chr(10), 0, 'C', 0);
                // $this->writeHTML('<strong>' . $this->tituloNombreCamara . '</strong>', true, false, true, false, 'C');
                // $this->SetXY(35, 15);
                $this->MultiCell(195, 4, $this->tituloRazonSocial . chr(13) . chr(10) . chr(13) . chr(10), 0, 'C', 0);
                $this->SetTextColor(0, 0, 0);
                //
                $txt = '<strong>Fecha expedición: </strong>' . date("Y/m/d") . ' - ' . date("H:i:s");
                if ($this->tituloRecibo != '') {
                ...
                if (
                    ...
                    if (date("Ymd") <= $_SESSION["generales"]["fcorte"]) {
                        ...
                        $txt .= 'RENUEVE SU INSCRIPCIÓN A MÁS TARDAR EL ' . strtoupper(\funcionesGenerales::mostrarFechaLetras1($_SESSION["generales"]["fcorte"])) . '.';
                        $this->writeHTML($txt, true, false, true, false, 'C');
                    } else {
                        $this->Ln();
                        $this->Ln();
    ...
    // Instanciamiento
    // ****************************************************************************** //
    $pdf = new PDFRequeEsadl1(PDF_PAGE_ORIENTATION, PDF_UNIT, 'LETTER', true, 'UTF-8', false, true);
    if (defined('TAMANO_LETRA_CERTIFICADOS_SII') && trim(TAMANO_LETRA_CERTIFICADOS_SII) != '') {
        $pdf->tamanoLetra = TAMANO_LETRA_CERTIFICADOS_SII;
    }
    $pdf->tituloEstadoMatricula = $data["estadomatricula"];
    ...
    if (substr($data["matricula"], 0, 1) != 'S') {
        ...
        if ($data["claseespesadl"] == '61') {
            $pdf->writeHTML('Con fundamento en las inscripciones del Registro de entidades extranjeras de derecho privado sin ánimo de lucro,', true, false, true, false, 'C');
        } else {
            $pdf->writeHTML('Con fundamento en las inscripciones del Registro de Entidades sin Ánimo de Lucro y de la Economía Solidaria,', true, false, true, false, 'C');
        }
    }
...
function generarCertificadosPdfLibros($mysqli, $data, $tipo, $valorCertificado = 0, $operacion = '', $recibo = '', $aleatorio = '', $tipoCertificado = '', $certificadoConsultaRues = 'no', $escajero = 'SI', $esbanco = 'NO', $firmar = '') {
    ...
    if (!class_exists('PDFRequeLib1')) {

        class PDFRequeLib1 extends TCPDF {
    ...
    // Instanciamiento
    // ****************************************************************************** //
    $pdf = new PDFRequeLib1(PDF_PAGE_ORIENTATION, PDF_UNIT, 'LETTER', true, 'UTF-8', false, true);
    if (defined('TAMANO_LETRA_CERTIFICADOS_SII') && trim(TAMANO_LETRA_CERTIFICADOS_SII) != '') {
        $pdf->tamanoLetra = TAMANO_LETRA_CERTIFICADOS_SII;
    }
    $pdf->tituloEstadoMatricula = $data["estadomatricula"];
    ...
    if ($pdf->tituloTipo == 'ConsultaD') {
        $name1 = $_SESSION["generales"]["pathabsoluto"] . '/tmp/' . $_SESSION["generales"]["codigoempresa"] . '-ConsultaCertificado-' . $aleatorio . '.pdf';
        $name2 = $_SESSION["generales"]["codigoempresa"] . '-ConsultaCertificado-' . $aleatorio . '.pdf';
        $pdf->Output($name1, "F");
        ob_end_clean();
        return $name2;
    }
...
function armarCertificaMatriculaInscripcion($pdf, $data, $titulo = '', $imprimirtitulo = 'si', $mysqli = null) {
    ...
    if ($data["organizacion"] == '01' || ($data["organizacion"] > '02' && $data["categoria"] == '1')) {
        if (!isset($data["ultimosactivosreportados"]) || $data["ultimosactivosreportados"] == null || $data["ultimosactivosreportados"] == '') {
            $data["ultimosactivosreportados"] = 0;
        }
        $txt .= '<strong>ACTIVO TOTAL  : </strong>' . number_format($data["ultimosactivosreportados"], 2) . '<br>';
        ...
        $txt .= '<strong>ACTIVO VINCULADO  : </strong>' . number_format((double) $data["ultimosactivosvinculados"], 2) . '<br>';
    }
...
function armarLibros($pdf, $data, $mysqli = null) {
    ...
    foreach ($data["inscripciones"] as $ix) {
        if (
            ...
            if ($ix["lib"] == 'RM91') {
                $txt1 .= '<td width="14%">' . $ix["libant"] . '-' . $ix["regant"] . '</td>';
            ...
            if ($data["disueltaporvencimiento"] == 'si') {
                ...
                if ($data["perdidacalidadcomerciante"] == 'si') {
                    if (trim($data["fechaperdidacalidadcomerciante"]) != '') {
                        $pdf->SetFont('courier', 'B', 8);
                        $pdf->SetTextColor(139, 0, 0);
                        $txt = '<strong>LAS PERSONAS NATURALES QUE HAN INSCRITO ACTO DE CESACIÓN DE ACTIVIDAD COMERCIAL NO TIENEN LA OBLIGACIÓN DE RENOVAR SU MATRÍCULA MERCANTIL, DESDE LA FECHA EN QUE SE INSCRIBIO EL ACTO EN CUESTIÓN. SIN EMBARGO DEBEN RENOVAR SU MATRÍCULA MERCANTIL HASTA EL AÑO EN QUE CESÓ SU ACTIVIDAD.</strong>';
                        $pdf->writeHTML($txt, true, false, true, false, 'C');
                        $pdf->SetFont('courier', '', 8);
                        $pdf->SetTextColor(0, 0, 0);
                        $pdf->Ln();
                                    ...
                                    if ($tx && !empty($tx)) {
                                        ...
                                        if (trim($v["municipiootros"]) != '') {
                                            if ($v["municipiootros"] == '00000' || $v["municipiootros"] == '99999') {
                                                ...
                                                $txt .= 'MUNICIPIO : ' . $v["municipiootros"] . ' - ' . retornarRegistroMysqliApi($mysqli, "bas_municipios", "codigomunicipio='" . $v["municipiootros"] . "'", "ciudad") . '<br>';
                                            }
                                        }
...
function armarCertificaTextoLibreClase($pdf, $data, $clase = '', $titulo = '', $titulos = 'si', $mysqli = null) {
    ...
    if ($clase != 'AC-SOCIOS' && $clase != 'CRT-REFORMAS' && $clase != 'CRT-REFORMAS-HTML' && $clase != 'CRT-VARIOS') {
        foreach ($_SESSION["generales"]["clasecerti"] as $certif => $dtax) {
            if ($dtax["clase"] == $clase) {
                if (isset($data["crtsii"][$certif]) && trim($data["crtsii"][$certif]) != '') {
                    ...
                    $txt1 = parsear_a_mayusculas($txt1);
        ...
        if ($txtx != '') {
            ...
            //
            // $txtx = parsear_a_mayusculas($txtx);
            //wsierra: 2019-12-18 : Limitar tamaño de parrafo y fraccionarlo CRT-OBJSOC
            $txtrev = '';
            if ($clase == 'CRT-OBJSOC') {
                if (strlen($txtx) >= 20000) {
                    // $txt = '<strong>' . $titulo . '</strong>';
                    // $pdf->writeHTML($txt, true, false, true, false, 'C');
                    // $pdf->Ln();
                    $txtrev = str_replace(array(chr(13) . chr(10), chr(13), chr(10)), "||", $txtx);
...
function armarCertificaReestructuracion($pdf, $data, $mysqli = null) {
    ...
    if ($libro19 == 'si' || $libro51 == 'si' || $libro53 == 'si') {
        if ($_SESSION["generales"]["TITULOS_EN_CERTIFICADOS_SII"] != 'NO') {
            $txt = '<strong>CERTIFICA - REORGANIZACIÓN, ADJUDICACIÓN O LIQUIDACIÓN JUDICIAL</strong>';
            $pdf->writeHTML($txt, true, false, true, false, 'C');
            $pdf->Ln();
        } else {
            $txt = '<strong>CERTIFICA</strong>';
        ...
        foreach ($data["inscripciones"] as $i) {
            ...
            if ($i["lib"] == 'RE51') {
                ...
                if ($i["crev"] != '1' && $i["crev"] != '9' && $i["grupoacto"] == '029') {
                    $txt = descripciones($mysqli, $data["organizacion"], $i["acto"], $i["tdoc"], $i["ndoc"], $i["ndocext"], $i["fdoc"], $i["idoridoc"], $i["txoridoc"], $i["idmunidoc"], $i["lib"], $i["nreg"], $i["freg"], $i["not"], '', '', '', $i["camant"], $i["libant"], $i["regant"], $i["fecant"]);
                    $pdf->writeHTML('<span style="text-align:justify;">' . $txt . '</span>', true, false, true, false);
                    $pdf->Ln();
                }
...
function armarCertificaVigencia($pdf, $data, $mysqli = null) {
    ...
    /*
      ...
      $txt = 'QUE LA VIGENCIA DE LA PERSONA JURIDICA ERA HASTA EL ' . strtoupper(\funcionesGenerales::mostrarFechaLetras1(trim($data["fechavencimiento"])));
      }
      } else {
      $estextual = 'si';
      }
...
function armarCertificaVigilanciaControl($pdf, $data, $mysqli = null) {
    ...
    if ($txtvig != '') {
        ...
        $txt = 'QUE LA ENTIDAD QUE EJERCE LA FUNCIÓN DE INSPECCIÓN, VIGILANCIA Y CONTROL ES ' . $txtvig;
        $pdf->writeHTML($txt, true, false, true, false, 'J');
        $pdf->Ln();
        return true;
    } else {
    ...
    if ($tieneestablecimientos == 'si') {
        ...
        foreach ($data["establecimientos"] as $e) {
            if ($e["estadodatosestablecimiento"] == 'MA' || $e["estadodatosestablecimiento"] == 'MI') {
                ...
                $txt .= '<br><strong>VALOR DEL ESTABLECIMIENTO : </strong>' . number_format((float) $e["actvin"]);
                if ($e["embargado"] == 'SI') {
...
function armarCertificaEstablecimientosArrendados($pdf, $data, $mysqli = null) {
    ...
    if ($tieneestablecimientos == 'si') {
        ...
        foreach ($data["establecimientosarrendados"] as $e) {
            ...
            $txt .= '<br><strong>VALOR DEL ESTABLECIMIENTO : </strong>' . number_format($e["actvin"]);
            $txt .= '<br>';
            $pdf->writeHTML($txt, true, false, true, false, 'J');
        }
    }
...
function armarCertificaTamanoEmpresarial957($pdf, $data, $mysqli = null) {

    // 
    if (defined('FECHA_INICIAL_CERTIFICA_TAMANO_957') && FECHA_INICIAL_CERTIFICA_TAMANO_957 != '' && FECHA_INICIAL_CERTIFICA_TAMANO_957 <= date("Ymd")) {
        if ($data["tamanoempresarial957"] != '') {
            ...
            $txtCuerpo .= 'Ingresos por actividad ordinaria : $' . number_format($data["ingresostamanoempresarial"], 0) . "\r\n";
            $txtCuerpo .= 'Actividad económica por la que percibió mayores ingresos en el periodo - CIIU : ' . $data["ciiutamanoempresarial"];
    ...
    if ($data["categoria"] == '1') {
        if (
            ...
            if ($data["organizacion"] == '04' || $data["organizacion"] == '07' || $data["organizacion"] == '16') {
                ...
                if ($data["cuoaut"] != 0) {
                    $txt .= '<td width="25%">' . truncarCuotas($data["cuoaut"]) . '</td>';
                } else {
                    $txt .= '<td width="25%">&nbsp;</td>';
                }
                ...
                if ($data["cuoaut"] != 0) {
                    if ($data["nomaut"] != 0) {
                        $tvalnom = truncarCuotas($data["nomaut"]);
                        $txt .= '<td width="25%">' . $tvalnom . '</td>';
                    } else {
                        $valnom = $data["capaut"] / $data["cuoaut"];
                        $tvalnom = truncarCuotas($valnom, 2);
                        $txt .= '<td width="25%">' . $tvalnom . '</td>';
                    }
                } else {
                    $txt .= '<td width="25%">&nbsp;</td>';
                ...
                if ($data["cuosus"] != 0) {
                    $txt .= '<td width="25%">' . truncarCuotas($data["cuosus"]) . '</td>';
                } else {
                    $txt .= '<td width="25%">&nbsp;</td>';
                }
                ...
                if ($data["cuosus"] != 0) {
                    if ($data["nomsus"] != 0) {
                        $tvalnom = truncarCuotas($data["nomsus"]);
                        $txt .= '<td width="25%">' . $tvalnom . '</td>';
                    } else {
                        $valnom = $data["capsus"] / $data["cuosus"];
                        $tvalnom = truncarCuotas($valnom);
                        $txt .= '<td width="25%">' . $tvalnom . '</td>';
                    }
                } else {
                    $txt .= '<td width="25%">&nbsp;</td>';
                ...
                if ($data["cuopag"] != 0) {
                    $txt .= '<td width="25%">' . truncarCuotas($data["cuopag"]) . '</td>';
                } else {
                    $txt .= '<td width="25%">&nbsp;</td>';
                }
                ...
                if ($data["cuopag"] != 0) {
                    if ($data["nompag"] != 0) {
                        $tvalnom = truncarCuotas($data["nompag"]);
                        $txt .= '<td width="25%">' . $tvalnom . '</td>';
                    } else {
            ...
            if ($data["organizacion"] == '08') {
                if (!isset($data["monedacap"]) || $data["monedacap"] == '' || $data["monedacap"] == '001') {
            ...
            if ($v["idtipoidentificacionotros"] == '2') {
                ...
                if ($v["idtipoidentificacionotros"] == '7') {
                    ...
                    if ($v["idtipoidentificacionotros"] == '5' || $v["idtipoidentificacionotros"] == 'E') {
                        ...
                        if (is_numeric($v["identificacionotros"]) && $v["idtipoidentificacionotros"] != 'V') {
                            $txt .= '<td width="20%">' . $ti . ' ' . number_format($v["identificacionotros"], 0) . '</td>';
                        } else {
                            $txt .= '<td width="20%">' . $ti . ' ' . $v["identificacionotros"] . '</td>';
                        }
...
function descripciones($mysqli, $organizacion, $acto, $tipdoc, $numdoc, $numdocext, $fecdoc, $idorigen, $txtorigen, $munori, $libro, $registro, $fecins, $noticia, $nan = array(), $nombre = '', $comple = '', $camant = '', $libant = '', $regant = '', $fecant = '') {
    ...
    if ($tipdoc != '38') {
        ...
        // 2020-07-27: JINT
        if ($txtSuscribe == 'HEREDERO') {
            $txtParticula = 'DEL';
            $txtSuscribe = 'HEREDERO';
            $noparticula = 'si';
        }

        // 2020-07-27: JINT
        if ($txtSuscribe == 'HEREDEROS') {
            $txtParticula = 'DE LOS';
            $txtSuscribe = 'HEREDEROS';
            $noparticula = 'si';
        }
        ...
        if ($noparticula != 'si') {
            ...
            if ($origenes && !empty($origenes)) {
                $txtParticula = strtoupper($origenes["campo1"]);
                $txtSuscribe = strtoupper($origenes["campo2"]);
                $noparticula = 'si';
            }
        }
    ...
    if ($acto == '1920' || $acto == '1921' || $acto == '1922') { // Resoluciones
        $txt .= 'SE RESOLVIÓ : ';
        $si = 'si';
    }
...
function descripcionesCambioNombre($mysqli, $tipdoc, $numdoc, $numdocext, $fecdoc, $idorigen, $txtorigen, $munori, $libro = '', $registro = '', $fecins = '', $noticia = '', $camant = '', $libant = '', $regant = '', $fecant = '', $organizacion = '', $categoria = '') {
    ...
    if ($noparticula != 'si') {
        ...
        if ($origenes && !empty($origenes)) {
            $txtParticula = strtoupper($origenes["campo1"]);
            $txtSuscribe = $txtParticula . ' ' . strtoupper($origenes["campo2"]);
            $noparticula = 'si';
        }
    }
...
function armarTextoElecciones($pdf, $dbx = null) {
    //
    /*
      if (
      date("Y") != '2022' &&
...
function armarTextoFirmaImpresion($pdf, $aleatorio) {
    ...
    if ($aleatorio != '') {
        $txt = 'No obstante, si usted va a imprimir este certificado, lo puede hacer desde su computador, con la certeza de que el mismo fue '
                ...
                . 'impreso, puede verificar hasta po 60 días y cuantas veces lo requiera, el contenido del mismo, ingresando al enlace https://sii.confecamaras.co/vista/plantilla/cv.php?empresa=' . CODIGO_EMPRESA . ' '
